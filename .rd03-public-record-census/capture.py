...
user_agent = 'BigBirdReturns-clifford-number-rd03-census/1.0 (https://github.com/BigBirdReturns/clifford-number; bigbirdreturns@users.noreply.github.com)'
...
def capture(route):
    ...
    for attempt_number in range(1, route['max_attempts'] + 1):
        ...
        command = [
            ...
            '--connect-timeout', '20', '--max-time', '90', '--max-filesize', '31457280',
            '--user-agent', user_agent,
            '--header', 'Accept: text/html,application/xhtml+xml,application/json,application/xml,text/xml,application/rss+xml,application/pdf,*/*;q=0.8',
...
(root / 'route-results.json').write_text(json.dumps({
    'schema_version': 'ssc-rd03-wave02-public-record-route-results@1',
    'research_head': plan['research_head'],
    'route_count': len(receipts),
    'routes': receipts,
}, indent=2) + '\n')
print(f"captured {len(receipts)} fixed RD-03 routes")
