...
validator_anchor = r"""    const interval = validatedIndependentPhoneInterval(
      candidate, groups, first, externalPrefix, externalSuffix, indeterminatePhoneContext
    );
"""
validator_replacement = r"""    const interval = validatedIndependentPhoneInterval(
      candidate,
      groups,
      first,
      externalPrefix,
...
if library.count(validator_anchor) != 1:
    raise SystemExit(
        f"unowned closer validator anchor count={library.count(validator_anchor)}"
    )
