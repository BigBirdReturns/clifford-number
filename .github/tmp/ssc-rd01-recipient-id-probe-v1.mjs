...
const OUT = 'rd01-recipient-id-probe-v1';
const names = ['Anduril', 'Saronic Technologies', 'Sierra Space', 'Shield AI', 'Anthropic'];
const awardTypeCodes = [
  'A','B','C','D','IDV_A','IDV_B','IDV_B_A','IDV_B_B','IDV_B_C','IDV_C','IDV_D','IDV_E',
  '02','03','04','05','06','07','08','09','10','11','-1'
];
const fields = [
  'Recipient Name', 'Recipient UEI', 'recipient_id', 'Recipient Location',
  'Award ID', 'generated_internal_id', 'prime_award_recipient_id'
];
...
const summary = {
  schema_version: 'ssc-rd01-recipient-id-probe@1',
  routes: names.length,
  names,
...
fs.writeFileSync(path.join(OUT, 'manifest.json'), JSON.stringify({ schema_version: 'ssc-rd01-recipient-id-probe-manifest@1', entries: files.length, combined_sha256, files }, null, 2) + '\n');
