...
observation_interval_replacement = r"""      if (!score) continue;
      ...
      // A proved observation owns every digit group through its exact source
      // endpoint. The ordinary optimizer must not restart at an internal group,
      // where observation digits can combine with a later local candidate.
