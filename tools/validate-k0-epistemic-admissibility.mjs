...
export function validateK0({
  ...
  if (JSON.stringify(method.selection_contract?.execution_wave_ids) !== JSON.stringify(['K0-W01','K0-W02','K0-W03','K0-W04','K0-W05','K0-W06','K0-W07'])) fail('method execution-wave drift');
  if (JSON.stringify(method.selection_contract?.query_templates_executed) !== JSON.stringify(['K0-Q01','K0-Q03','K0-Q04','K0-Q06','K0-Q08','K0-Q05','K0-Q07','K0-Q09'])) fail('method query-template execution drift');
  ...
  if (neutral.status !== 'execution_started_wave_07_field_pending' || neutral.execution.name_blind_execution_started !== true || neutral.execution.searches_executed !== 44 || neutral.execution.query_templates_executed !== 8 || neutral.execution.raw_results_observed !== 206 || neutral.execution.returned_records !== 57 || neutral.execution.included_events !== 0) fail('neutral execution state drift');
  if (neutral.execution.candidate_records !== 24 || neutral.execution.positive_controls !== 13 || neutral.execution.negative_controls !== 10 || neutral.execution.coverage_controls !== 7 || neutral.execution.requires_additional_acquisition !== 3 || neutral.execution.resolved_additional_acquisition !== 2 || neutral.execution.open_additional_acquisition !== 1 || neutral.execution.non_events !== 30) fail('neutral execution classification drift');
  if (JSON.stringify(neutral.execution.executed_wave_ids) !== JSON.stringify(['K0-W01','K0-W02','K0-W03','K0-W04','K0-W05','K0-W06','K0-W07']) || neutral.execution.independent_second_party_review_complete !== false) fail('neutral wave/independence drift');
  ...
  if (denominatorWave07?.status !== 'discovery_complete_field_adjudication_pending') fail('Wave 07 denominator state drift');
  ...
  if (pendingFieldMetric?.observed !== 3 || pendingFieldMetric?.source !== 'data/research/k0-role-neutral-wave-07.json') fail('Wave 07 coverage metric drift');
  ...
  if (wave07CoverageGap?.status !== 'open') fail('Wave 07 coverage gap drift');
  ...
  if (authorshipComparator?.status !== 'discovery_complete_field_pending' || !authorshipComparator.blocking_conditions?.some(value => /(?:field|stage) adjudication/i.test(value))) fail('Wave 07 comparator review drift');
