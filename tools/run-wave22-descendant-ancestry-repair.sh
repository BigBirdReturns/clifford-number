...
trigger='.github/tmp/wave22-descendant-ancestry-repair-trigger.json'
temporary_workflow='.github/workflows/temporary-wave22-descendant-ancestry-repair.yml'
runner='tools/run-wave22-descendant-ancestry-repair.sh'
...
replaceBlock(
  ...
  ancestryBlock(
    'LAW22_SKIP_GIT',
    'Wave 22 base checkpoint unavailable after targeted full-history recovery',
...
replaceBlock(
  ...
  ancestryBlock(
    'LAW23_SKIP_GIT',
    'Wave 23 base checkpoint unavailable after targeted full-history recovery',
...
const methodUpdates = [
  {
    ...
    after: 'Ancestry validation distinguishes an unavailable commit object from failed ancestry. In a depth-one GitHub pull-request merge checkout, the validator performs a targeted `--unshallow` fetch of `GITHUB_HEAD_REF` into an explicit remote-tracking ref, then reruns the same merge-base ancestry test against that recovered feature-branch tip. This recovers ancestry across descendant feature branches while preserving the original predicate. A checkpoint missing after targeted full-history recovery or a recovered non-ancestor remains blocking.'
  },
  {
    path: 'docs/methods/lake-allocator-war-lead-acquisition-wave-23.md',
    before: 'Ancestry validation is portable across full-history checkouts and depth-one pull-request merge checkouts. When the declared base commit is absent in GitHub Actions, the validator fetches `GITHUB_HEAD_REF` into an explicit remote-tracking ref and tests the same ancestry predicate against that recovered feature-branch tip. Missing ancestry after targeted recovery and genuine non-ancestry remain blocking.',
    after: 'Ancestry validation is portable across full-history checkouts and depth-one pull-request merge checkouts. In GitHub Actions, the validator performs a targeted `--unshallow` fetch of `GITHUB_HEAD_REF` into an explicit remote-tracking ref and tests the same ancestry predicate against that recovered feature-branch tip. This remains valid when Wave 23 is itself a descendant of an earlier feature merge. Missing ancestry after targeted full-history recovery and genuine non-ancestry remain blocking.'
  }
];
...
rm -f \
  .github/workflows/temporary-wave22-descendant-ancestry-diagnostic.yml \
  .github/tmp/wave22-descendant-ancestry-diagnostic-trigger.json \
  "$trigger" \
  "$temporary_workflow" \
  "$runner"
...
for pass in 1 2 3; do
  fixed_point_pass "Wave 22 descendant ancestry repair pass $pass"
done
validate_epoch
npm run release:check
for pass in 1 2 3; do
  fixed_point_pass "Wave 22 descendant ancestry post-release seal pass $pass"
done
validate_epoch
...
test ! -e .github/workflows/temporary-wave22-descendant-ancestry-diagnostic.yml
test ! -e .github/tmp/wave22-descendant-ancestry-diagnostic-trigger.json
test ! -e "$trigger"
test ! -e "$temporary_workflow"
test ! -e "$runner"
...
git commit -m 'Recover descendant PR history before ancestry validation'
