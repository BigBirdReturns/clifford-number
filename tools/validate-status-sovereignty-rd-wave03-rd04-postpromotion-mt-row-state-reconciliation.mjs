...
`)),'manifest canonical serialization mismatch');
  ...
  const workflow=readBytes(repoRoot,'.github/workflows/status-sovereignty-rd-wave03-rd04-postpromotion-mt-row-state-reconciliation.yml',overrides).toString();assert(workflow.includes(`CANONICAL_PARENT: ${C.CANONICAL_PARENT}`)&&workflow.includes('first-parent-edge-paths.tsv')&&workflow.includes('diff-filter=A'),'workflow control mismatch');assert(workflow.includes('git cat-file commit \"$PRODUCT_COMMIT\"')&&workflow.includes('git hash-object -t commit --stdin < \"$EXPECTED_COMMIT_OBJECT\"')&&workflow.includes('cmp \"$EXPECTED_COMMIT_OBJECT\" \"$ACTUAL_COMMIT_OBJECT\"')&&!workflow.includes("--format='%aI'")&&!workflow.includes("--format='%cI'")&&!workflow.includes("--format='%B' \"$PRODUCT_COMMIT\""),'workflow exact commit-object control mismatch');
  assert(workflow.includes('QUALIFIED_PRODUCT_REF: refs/heads/lock/ssc-rd04-mt-row-state-reconciliation-v4-qualified')&&workflow.includes('refs/remotes/origin/qualified-product')&&workflow.includes('git rev-parse refs/remotes/origin/qualified-product')&&workflow.includes('git ls-tree \"$PRODUCT_COMMIT\"')&&workflow.includes("'100644 blob'"),'workflow external qualification-ref and path-mode control mismatch');
