...
const jsonPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v13.json';
const markdownPath = process.argv[4] ?? 'build/research/preference-custody-laboratory-floor-v13.md';
...
const federationBuild = compilePreferenceTrustFederationFixture(
  readJson('data/research/preference-custody/trust-federation.fixture.json')
);
const compiled = compilePreferenceCustodyManifestV13(readJson(manifestPath), baseV12, federationBuild);
mkdirSync(dirname(jsonPath), { recursive: true });
mkdirSync(dirname(markdownPath), { recursive: true });
writeFileSync(jsonPath, JSON.stringify(compiled, null, 2) + '\n');
writeFileSync(markdownPath, renderPreferenceCustodyManifestV13Markdown(compiled));
