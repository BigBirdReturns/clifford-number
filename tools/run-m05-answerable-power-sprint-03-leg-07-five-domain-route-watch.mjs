...
import {runRouteWatch,validateContract,validateReceipt} from './lib/m05-answerable-power-sprint-03-leg-07-five-domain-route-watch.mjs';

const DEFAULT_CONTRACT='data/project/m05-answerable-power-sprint-03-leg-07-five-domain-route-watch-contract.json';
const DEFAULT_OUTPUT='build/m05-five-domain-route-watch/m05-five-domain-route-watch-receipt.json';

function usage(){
  return `Usage: node tools/run-m05-answerable-power-sprint-03-leg-07-five-domain-route-watch.mjs [options]\n\nOptions:\n  --contract <path>     Contract JSON path (default: ${DEFAULT_CONTRACT})\n  --output <path>       Receipt JSON path (default: ${DEFAULT_OUTPUT})\n  --previous <path>     Optional prior receipt for byte/status comparison\n  --observed-at <time>  Historical RFC 3339 observation clock; future values are refused\n  --help                Show this help\n`;
}
...
function parseArgs(argv){
  ...
  for(let index=0;index<argv.length;index++){
    ...
    if(!['--contract','--output','--previous','--observed-at'].includes(token))throw new Error(`Unknown argument: ${token}`);
...
async function main(){
  ...
  process.stdout.write(JSON.stringify({receipt:args.output,summary:summaryPath,proof:proofPath,observation_clock_utc:receipt.observation_clock_utc,...receipt.summary},null,2)+'\n');
}

main().catch((error)=>{
  process.stderr.write(`five-domain route watch failed: ${error?.stack||error}\n`);
  process.exitCode=1;
});
