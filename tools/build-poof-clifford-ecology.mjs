...
function renderRoutes(data) {
  ...
  const objectCards = data.objects.objects.map((row) => `<article class="card object"><p class="eyebrow">${escapeHtml(row.object_id)}</p><h3>${escapeHtml(row.schema_version)}</h3><p>${escapeHtml(row.authority)}</p>${pills([`canonical write: ${row.canonical_write}`, `graph effect: ${row.graph_effect}`])}</article>`).join('');
  ...
  const methodsBody = `${hero('Constitution and method', 'Projection is not promotion', 'The ecology adds public usability, reciprocal challenge, and machine access while leaving evidentiary authority in Clifford.')}<section class="section"><div class="wrap"><h2>One-way law</h2><p class="law">${escapeHtml(data.contract.governing_law.statement)}</p><div class="grid"><article class="card wide"><h3>Evidence authority moves outward</h3>${list(data.contract.governing_law.evidence_outward)}</article><article class="card wide"><h3>Challenges move inward</h3>${list(data.contract.governing_law.challenge_inward)}</article><article class="card full"><h3>Prohibited transformations</h3>${list(data.contract.governing_law.no_reverse_authority)}</article></div></div></section><section class="section"><div class="wrap"><h2>Five transaction schemas</h2><div class="grid">${objectCards}</div></div></section><section class="section"><div class="wrap"><h2>Hard refusals</h2><pre>POOF prose ≠ canonical fact\nreader reaction ≠ finding\nWebsiteIQ score ≠ substantive truth\nsuccessful onboarding ≠ endorsement\nREAL STEEL fixture ≠ neutral denominator\nSteel Mirror differential ≠ bad faith\nMCP narration ≠ evidence</pre></div></section>`;
