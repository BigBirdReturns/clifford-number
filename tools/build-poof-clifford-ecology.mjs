...
function renderRoutes(data) {
  ...
  const actionRoutes = { 'Read the argument':'/report/', 'Inspect a decision':'/estate/', 'Test a claim':'/examination/', 'Use the newsroom desk':'/newsroom/', 'Examine the evidence':'/examination/', 'Challenge the model':'/methods/', 'Query as an agent':'/machine/' };
  ...
  const examBody = `${hero('POOF jurisdiction 3', 'Examination', 'External disciplines, subject responses, counterevidence, and publication audits can compel a question to reopen without becoming facts by declaration.')}<section class="section"><div class="wrap"><h2>Proceedings 13 through 16</h2><div class="grid">${examinationCards}</div></div></section><section class="section interactive"><div class="wrap"><h2>Prepare a graph-inert referral</h2><p class="muted">This form exports a local packet. It does not submit, publish, or write to canonical data.</p><form id="referral-form" class="grid"><div class="card wide">${[['ref-proposition','Exact proposition',15],['ref-ceiling','Current evidence ceiling',8],['ref-record','Required record or test',8],['ref-route','Lawful acquisition route',8]].map(([id,label,minLength]) => `<div class="field"><label for="${id}">${label}</label><textarea id="${id}" required minlength="${minLength}"></textarea></div>`).join('')}</div><div class="card wide">${[['ref-custodian','Responsible custodian',3],['ref-consequence','Consequence if unresolved',8],['ref-privacy','Privacy boundary',8]].map(([id,label,minLength]) => `<div class="field"><label for="${id}">${label}</label><textarea id="${id}" required minlength="${minLength}"></textarea></div>`).join('')}<button class="button primary" type="button" id="referral-export">Export referral packet</button><p id="referral-result" class="result" role="status" aria-live="polite" hidden></p></div></form></div></section>`;
