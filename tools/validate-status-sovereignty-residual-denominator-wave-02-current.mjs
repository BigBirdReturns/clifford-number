...
export function validateCurrentShape(current, schema) {
  ...
  ok(current.authority === 'six_terminal_class_receipts_promoted_without_cross_lane_empirical_authority', 'current authority changed');
  ...
  ok(Array.isArray(current.promoted_class_receipts) && current.promoted_class_receipts.length === 6, 'five promoted class receipts required');
  ...
  same(current.promoted_class_receipts.map((row) => row.class_id), ['RD-04-C01','RD-05-C03','RD-01-C03','RD-06-C01','RD-03-C04','RD-02-C04'], 'promoted class order changed');
  ...
  const rd02 = current.promoted_class_receipts[5];
  ...
  ok(rd02.lane_id === 'RD-02' && rd02.issue === 787 && rd02.source_pr === 802, 'RD-02 custody changed');
  ok(rd02.merge_commit === '72abc35c408d172d9be33b619b630a96ac317193', 'RD-02 merge custody changed');
  ok(rd02.manifest_combined_sha256 === '0ca72d32840bf079975448fa9e9de3f75cdad68555c085f7f0749d007c1dc427', 'RD-02 manifest custody changed');
  ok(rd02.terminal_state === 'bounded_source_unavailable', 'RD-02 terminal state changed');
  ok(rd02.constitutional_exact_label === 'fund-level Green Light, license, leverage commitment, draw, fee, covenant, and amendment chronology' && rd02.receipt_class_label === 'fund-level Green Light, license, leverage commitment, draw, fee, covenant, and amendment chronology', 'RD-02 exposed label changed');
  ok(rd02.labels_exact_match === true && rd02.label_reconciliation === 'none', 'RD-02 flattened label state changed');
  ok(rd02.closure_reference_path === 'data/project/ssc-residual-wave02/closures/RD-02-C04.json' && rd02.class_receipt_path === 'data/research/status-sovereignty-rd-wave02-rd02-license-leverage/class-receipt.json', 'RD-02 path custody changed');

  ok(Array.isArray(current.selected_classes_open) && current.selected_classes_open.length === 0, 'no selected class may remain open');
  unique(current.selected_classes_open.map((row) => row.class_id), 'duplicate open selected class');
  same(current.selected_classes_open.map((row) => row.class_id), [], 'open selected class order changed');
  ...
  ok(current.counts.terminal_class_receipts === 6, 'terminal receipt count changed');
  ok(current.counts.classes_closed_this_wave === 6 && current.counts.closed_residual_classes === 6, 'closed class accounting changed');
  ok(current.counts.open_residual_classes === 36, 'open class accounting changed');
  ...
  ok(current.current_result.classes_closed === 6 && current.current_result.classes_open === 36, 'current result arithmetic changed');
  same(current.current_result.closed_class_ids, ['RD-04-C01','RD-05-C03','RD-01-C03','RD-06-C01','RD-03-C04','RD-02-C04'], 'closed class ids changed');
  same(current.current_result.open_selected_class_ids, [], 'open selected ids changed');
  ok(current.current_result.all_six_selected_classes_closed === true && current.current_result.wave_complete === false, 'selected-attempt completion or Wave-02 boundary changed');
  ...
  ok(schema?.properties?.selected_classes_open?.minItems === 0 && schema?.properties?.selected_classes_open?.maxItems === 0, 'schema open-class denominator changed');
