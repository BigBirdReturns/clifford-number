...
const report={
  schema_version:'m05-answerable-power-sprint-03-leg-02-report@1',
  ...
  counts:{
    ...
    administrative_invariants:stress.domain_adapter.administrative_invariants.length,
    ...
    kw_acquisitions:stress.decisive_acquisition.kw.length,
    robodebt_acquisitions:stress.decisive_acquisition.robodebt.length,
...
write('reports/core-thesis/answerable-power/sprint-03-leg-02.json',JSON.stringify(report,null,2)+'\n');
...
const levelRows=stress.r_level_tests.map((row)=>`<tr><td><code>${esc(row.level)}</code></td><td>${esc(row.required_by_apc)}</td><td><code>${esc(row.kw_state)}</code><br>${esc(row.kw_observation)}</td><td><code>${esc(row.robodebt_state)}</code><br>${esc(row.robodebt_observation)}</td><td><code>${esc(row.domain_result)}</code></td></tr>`).join('');
...
const html=`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Administrative benefits stress test</title><style>body{font:16px/1.55 system-ui;max-width:1500px;margin:36px auto;padding:0 22px;background:#ece9df;color:#171717}code,pre{font-family:ui-monospace,SFMono-Regular,monospace}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.metric,.box,table{background:#fffdf7;border:1px solid #c9c1b2;border-radius:12px}.metric,.box{padding:15px}.metric b{display:block;font-size:1.8rem}table{border-collapse:collapse;width:100%;overflow:hidden}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left;vertical-align:top}.state{font-weight:800;color:#a43a00}.boundary{border-left:5px solid #8a2c24}</style></head><body><p><b>CLIFFORD NUMBER · M-05 · S03-L2</b></p><h1>Administrative benefits and automated burden</h1><p class="state">BOUNDED DOMAIN TEST · COMPOSED ANSWER NOT YET OBSERVED</p><div class="metrics"><div class="metric"><b>${report.counts.systems}</b>systems</div><div class="metric"><b>${report.counts.r_levels}</b>R-level tests</div><div class="metric"><b>${report.counts.administrative_invariants}</b>domain invariants</div><div class="metric"><b>${report.counts.action_packet_fields}</b>action-packet fields</div><div class="metric"><b>${report.counts.cross_system_controls}</b>controls</div><div class="metric"><b>${report.counts.propositions}</b>propositions</div></div><h2>Governing question</h2><pre class="box">${esc(stress.governing_question)}</pre><h2>Systems</h2><table><tr><th>System</th><th>Case</th><th>Jurisdiction</th><th>Observed control</th><th>Highest</th><th>Largest limit</th></tr>${systemRows}</table><h2>Benefits and Debt Covenant</h2><pre class="box">${esc(JSON.stringify(stress.domain_adapter,null,2))}</pre><h2>R1–R7 stress matrix</h2><table><tr><th>Level</th><th>APC requirement</th><th>K.W.</th><th>Robodebt</th><th>Domain result</th></tr>${levelRows}</table><h2>Action-packet translation</h2><pre class="box">${esc(JSON.stringify(stress.action_packet_translation,null,2))}</pre><h2>Cross-system controls</h2><table><tr><th>ID</th><th>Claim</th><th>Disposition</th><th>Limit</th></tr>${controlRows}</table><h2>Decisive acquisition</h2><pre class="box">${esc(JSON.stringify(stress.decisive_acquisition,null,2))}</pre><h2>Proposition ledger</h2><table><tr><th>ID</th><th>Claim</th><th>Disposition</th><th>Ceiling</th></tr>${propositionRows}</table><h2>Current result</h2><pre class="box">${esc(JSON.stringify(stress.current_result,null,2))}</pre><h2>Boundary</h2><pre class="box boundary">${esc(JSON.stringify(stress.boundaries,null,2))}</pre></body></html>`;
write('reports/core-thesis/answerable-power/sprint-03-leg-02.html',html+'\n');
console.log(`build-m05-answerable-power-sprint-03-leg-02: ${report.counts.systems} systems, ${report.counts.r_levels} R-level tests, ${report.counts.propositions} propositions`);
