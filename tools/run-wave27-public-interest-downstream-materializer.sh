...
branch='agent/lake-allocator-war-public-interest-wave-27'
base='a936a8fc3d9f4527648779004d5899bf9cc17e68'
trigger='.github/tmp/wave27-public-interest-downstream-trigger.json'
temporary_workflow='.github/workflows/temporary-wave27-public-interest-downstream-materializer.yml'
...
const wave27PolicyPath = 'data/project/lake-allocator-war-public-interest-downstream-wave-27-policy.json';
const wave27 = readJson(wave27PolicyPath);
const publicResult = wave27.paths.result_root + '/law21-est-05.jsonl';
const legislativeResult = wave27.paths.result_root + '/law21-est-11.jsonl';
const required = {
  'allocator-war-source': [wave27PolicyPath, wave27.paths.method, wave27.paths.milestone, publicResult, legislativeResult],
  'allocator-war-lake-actions': [wave27.paths.execution_plan, wave27.paths.projection],
  'allocator-war-reports': [wave27.paths.report]
};
...
for (const relative of [wave27.paths.execution_plan, wave27.paths.projection, publicResult, legislativeResult]) {
  addSorted(policy.projection_contract.allowed_generated_paths, relative);
}
policy.boundaries.wave_27_downstream_result_is_evidence_row = false;
policy.boundaries.wave_27_formal_category_is_complete_affected_roster = false;
policy.boundaries.wave_27_formal_consequence_authority_is_observed_use = false;
policy.boundaries.wave_27_bounded_correction_control_is_systemwide_remedy_adequacy = false;
policy.boundaries.wave_27_legislative_blocked_task_may_receive_plan = false;
...
lakePolicy.boundaries.wave_27_downstream_execution_proves_evidence = false;
lakePolicy.boundaries.wave_27_formal_category_proves_complete_affected_roster = false;
lakePolicy.boundaries.wave_27_formal_consequence_authority_proves_observed_use = false;
writeJson(lakePolicyPath, lakePolicy);

const packagePath = 'package.json';
const pkg = readJson(packagePath);
pkg.scripts['build:lake-allocator-war-public-interest-downstream-wave-27'] = 'node tools/build-lake-allocator-war-public-interest-downstream-wave-27.mjs';
pkg.scripts['validate:lake-allocator-war-public-interest-downstream-wave-27'] = 'node tools/validate-lake-allocator-war-public-interest-downstream-wave-27.mjs && node test/lake-allocator-war-public-interest-downstream-wave-27.test.js';
pkg.scripts['ci:lake-allocator-war-public-interest-downstream-wave-27'] = 'npm run build:lake-allocator-war-public-interest-downstream-wave-27 && npm run validate:lake-allocator-war-public-interest-downstream-wave-27';
if (!pkg.scripts.check.includes('validate:lake-allocator-war-public-interest-downstream-wave-27')) {
  const marker = 'npm run validate:lake-allocator-war-wave26-source-custody-repair';
  if (!pkg.scripts.check.includes(marker)) throw new Error('Wave 26 source-custody release marker missing');
  pkg.scripts.check = pkg.scripts.check.replace(marker, marker + ' && npm run validate:lake-allocator-war-public-interest-downstream-wave-27');
}
...
const buildMarker = '3.27 **Allocator-war public-interest downstream execution — Wave 27.**';
...
const readmeMarker = '## Allocator-war public-interest downstream Wave 27';
...
node --check tools/build-lake-allocator-war-public-interest-downstream-wave-27.mjs
node --check tools/validate-lake-allocator-war-public-interest-downstream-wave-27.mjs
node --check test/lake-allocator-war-public-interest-downstream-wave-27.test.js
...
first_projection="$(sha256sum build/lake-actions/allocator-war-public-interest-downstream-wave-27.json | cut -d' ' -f1)"
first_plan="$(sha256sum build/lake-actions/allocator-war-public-interest-downstream-wave-27-source-plan.json | cut -d' ' -f1)"
first_results="$(find data/acquisition/lake-allocator-war-wave-27 -type f -name '*.jsonl' -print0 | sort -z | xargs -0 sha256sum | sha256sum | cut -d' ' -f1)"
first_report="$(sha256sum reports/lake-allocator-war-public-interest-downstream-wave-27.md | cut -d' ' -f1)"
node tools/build-lake-allocator-war-public-interest-downstream-wave-27.mjs
test "$first_projection" = "$(sha256sum build/lake-actions/allocator-war-public-interest-downstream-wave-27.json | cut -d' ' -f1)"
test "$first_plan" = "$(sha256sum build/lake-actions/allocator-war-public-interest-downstream-wave-27-source-plan.json | cut -d' ' -f1)"
test "$first_results" = "$(find data/acquisition/lake-allocator-war-wave-27 -type f -name '*.jsonl' -print0 | sort -z | xargs -0 sha256sum | sha256sum | cut -d' ' -f1)"
test "$first_report" = "$(sha256sum reports/lake-allocator-war-public-interest-downstream-wave-27.md | cut -d' ' -f1)"
git add -A
git diff --check
...
fixed_point_pass() {
  ...
  LAW27_SKIP_GIT=1 LAW27_SKIP_SHARDS=1 node tools/validate-lake-allocator-war-public-interest-downstream-wave-27.mjs
  git add -A
}

for pass in 1 2 3; do
  fixed_point_pass "Wave 27 public-interest fixed-point pass $pass"
done
...
for pass in 1 2 3; do
  fixed_point_pass "Wave 27 public-interest post-release seal pass $pass"
done
validate_epoch
...
git commit -m 'Materialize allocator-war public-interest downstream Wave 27'
