...
const jsonPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v14.json';
const markdownPath = process.argv[4] ?? 'build/research/preference-custody-laboratory-floor-v14.md';
...
const liabilityBuild = compilePreferenceLiabilityRemedyFixture(
  readJson('data/research/preference-custody/liability-remedy.fixture.json')
);
const compiled = compilePreferenceCustodyManifestV14(readJson(manifestPath), baseV13, liabilityBuild);
mkdirSync(dirname(jsonPath), { recursive: true });
mkdirSync(dirname(markdownPath), { recursive: true });
writeFileSync(jsonPath, JSON.stringify(compiled, null, 2) + '\n');
writeFileSync(markdownPath, renderPreferenceCustodyManifestV14Markdown(compiled));
