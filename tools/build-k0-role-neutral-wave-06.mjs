...
export function computeWave06Manifest() {
  ...
  return {
    ...
    boundaries: {
      ...
      reprisal_finding_proves_complete_k0_chain: false,
      professional_disagreement_proves_reclassification: false,
      lawful_discipline_proves_ceiling_conversion: false,
      stay_proves_final_merits: false,
...
const manifest = computeWave06Manifest();
const outcomes = wave.records.reduce((acc, row) => ((acc[row.selection_outcome] = (acc[row.selection_outcome] || 0) + 1), acc), {});
const report = {
  schema_version: 'k0-role-neutral-wave-06-report@1',
  program_id: 'M-05',
  layer_id: 'K0',
  wave_id: wave.wave_id,
  title: 'K0 role-neutral denominator Wave 06 · professional judgment and obstruction',
  status: wave.status,
  as_of: wave.as_of,
  ...
  release_manifest: {
    path: 'data/project/k0-role-neutral-wave-06-release-manifest.json',
    combined_sha256: manifest.combined_sha256
  },
  boundaries: wave.boundaries
};

write('data/project/k0-role-neutral-wave-06-release-manifest.json', stable(manifest));
write('reports/core-thesis/answerable-power/k0-role-neutral-wave-06.json', stable(report));
...
const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>K0 Wave 06 · professional judgment</title><style>body{font:16px/1.55 system-ui;max-width:1450px;margin:36px auto;padding:0 22px;background:#ece9df;color:#171717}code,pre{font-family:ui-monospace,SFMono-Regular,monospace}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px}.metric,.box,table{background:#fffdf7;border:1px solid #c9c1b2;border-radius:12px}.metric,.box{padding:15px}.metric b{display:block;font-size:1.8rem}table{border-collapse:collapse;width:100%}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left;vertical-align:top}.state{font-weight:800;color:#a43a00}.boundary{border-left:5px solid #8a2c24}</style></head><body><p><b>CLIFFORD NUMBER · M-05 · K0 · WAVE 06</b></p><h1>Professional-judgment, insubordination, and obstruction discovery</h1><p class="state">DISCOVERY COMPLETE · FIELD ADJUDICATION PENDING · PUBLICATION BLOCKED · GRAPH INERT</p><div class="metrics"><div class="metric"><b>${wave.counts.query_executions}</b>query executions</div><div class="metric"><b>${wave.counts.raw_results_observed}</b>raw results</div><div class="metric"><b>${wave.counts.retained_records}</b>retained records</div><div class="metric"><b>${wave.counts.candidate_requires_field_audit}</b>candidates</div><div class="metric"><b>${wave.counts.positive_controls}</b>positive controls</div><div class="metric"><b>${wave.counts.negative_controls}</b>negative controls</div><div class="metric"><b>${wave.counts.included_events}</b>included events</div></div><h2>Records</h2><table><tr><th>ID</th><th>Institution</th><th>Outcome</th><th>Field state</th></tr>${rows}</table><h2>Source-plane boundary</h2><pre class="box boundary">${esc(JSON.stringify(wave.source_plane, null, 2))}</pre><h2>Current result</h2><pre class="box">${esc(JSON.stringify(report.current_result, null, 2))}</pre><p><code>release SHA-256: ${manifest.combined_sha256}</code></p></body></html>`;
write('reports/core-thesis/answerable-power/k0-role-neutral-wave-06.html', html + '\n');

console.log(`build-k0-role-neutral-wave-06: ${wave.counts.retained_records} records, ${wave.counts.candidate_requires_field_audit} candidates, ${wave.counts.positive_controls} positive controls, 0 included events`);
