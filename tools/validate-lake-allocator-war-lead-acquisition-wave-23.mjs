...
export function validateRepository(root = defaultRoot) {
  ...
  if (process.env.LAW23_SKIP_GIT !== '1') {
    ...
    if (!baseAvailable && process.env.GITHUB_ACTIONS === 'true' && remoteHeadRef) {
      try {
        quietGit(['fetch', '--no-tags', 'origin', '+refs/heads/' + githubHeadRef + ':' + remoteHeadRef]);
      } catch {
        // The bounded availability check below records targeted recovery failure.
      }
      if (hasCommit(remoteHeadRef)) ancestryTarget = remoteHeadRef;
      baseAvailable = hasCommit(baseCommit);
    }
    if (!baseAvailable) {
      fail(errors, 'Wave 23 base checkpoint unavailable after explicit head-ref recovery');
    } else {
      try {
