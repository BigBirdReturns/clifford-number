...
export function computeSg04Manifest() {
  const entries = releaseScope.map((rel) => {
    const bytes = readBytes(rel);
    return { path: rel, sha256: sha256(bytes), bytes: bytes.length };
  });
  return {
    schema_version: 'project-stable-ground-sg04-release-manifest@1',
    checkpoint_id: 'SG-2026-07-29-04',
    as_of: '2026-07-29',
    ...
    boundaries: {
      ...
      manifest_proves_K0_event: false,
...
export function buildStableGroundSG04() {
  const checkpoint = read('data/project/project-stable-ground-sg04.json');
  ...
  const waveRelease = read('data/project/k0-role-neutral-wave-08-release-manifest.json');
  const poofRelease = read('data/project/poof-clifford-ecology-release-manifest.json');
  const manifest = computeSg04Manifest();

  write('data/project/project-stable-ground-sg04-release-manifest.json', stable(manifest));

  const snapshot = checkpoint.canonical_snapshot;
  const report = {
    schema_version: 'project-stable-ground-sg04-report@1',
    project_id: checkpoint.project_id,
    program_id: checkpoint.program_id,
    checkpoint_id: checkpoint.checkpoint_id,
    as_of: checkpoint.as_of,
    title: 'Stable-ground supersession 04 · Complete K0 query battery',
    ...
    counts: {
      ...
      k0_searches: snapshot.k0.searches_executed,
      k0_raw_results: snapshot.k0.raw_results_observed,
      k0_retained_records: snapshot.k0.returned_records,
      k0_candidate_records: snapshot.k0.candidate_records,
      wave_08_retained_records: snapshot.wave_08.retained_records,
      wave_08_candidate_records: snapshot.wave_08.candidate_records,
      included_k0_events: snapshot.k0.included_events,
    ...
    release_manifest: {
      path: 'data/project/project-stable-ground-sg04-release-manifest.json',
      combined_sha256: manifest.combined_sha256
    }
  };
  write('reports/core-thesis/stable-ground/sg04/checkpoint.json', stable(report));
...
<title>SG-04 · Complete K0 query battery · Clifford Number</title>
<style>
...
<h1>Complete search battery, unchanged evidentiary ceiling</h1>
<p class="state">CURRENT CHECKPOINT: ${esc(checkpoint.checkpoint_id)} · K0 EVENTS: 0 · EXTERNAL ADOPTION: A0</p>
...
<div class="grid">
  ...
  <div class="card"><b>${report.counts.k0_searches}</b>search executions</div>
  <div class="card"><b>${report.counts.k0_retained_records}</b>retained records</div>
  <div class="card"><b>${report.counts.wave_08_candidate_records}</b>Wave 08 candidates</div>
  <div class="card"><b>${report.counts.included_k0_events}</b>included K0 events</div>
...
<p class="small"><code>K0 release SHA-256: ${esc(k0Release.combined_sha256)}</code></p>
<p class="small"><code>Wave 08 release SHA-256: ${esc(waveRelease.combined_sha256)}</code></p>
<p class="small"><code>POOF release SHA-256: ${esc(poofRelease.combined_sha256)}</code></p>
<p class="small"><code>SG-04 release SHA-256: ${esc(manifest.combined_sha256)}</code></p>
</body>
</html>`;
  write('reports/core-thesis/stable-ground/sg04/index.html', `${html}\n`);

  console.log(
    `build-project-stable-ground-sg04: ${pointer.history.length} checkpoints, K0 ${snapshot.k0.query_templates_executed}/` +
    `${snapshot.k0.query_templates_total}, ${snapshot.k0.returned_records} retained, ${snapshot.k0.included_events} events, ` +
...
if (invoked) buildStableGroundSG04();
