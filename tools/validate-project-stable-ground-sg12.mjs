...
import { computeReleaseManifest as computePoofCliffordManifest } from './build-poof-clifford-ecology.mjs';
...
eq(currentRow?.transition_base, trigger.transition_base, 'SG-12 pointer transition base receipt');
  ...
  eq(trigger.checkpoint_campaign_release_sha256, checkpoint.authority_change?.campaign_release_sha256, 'campaign historical checkpoint custody');
  eq(JSON.stringify(campaignManifest), JSON.stringify(computeW02SPRManifest()), 'campaign current release exact-byte manifest');
  ...
  eq(trigger.checkpoint_poof_release_sha256, checkpoint.authority_change?.poof_release_sha256, 'POOF historical checkpoint custody');
  eq(JSON.stringify(poofManifest), JSON.stringify(computePoofCliffordManifest()), 'POOF current release exact-byte manifest');
