...
import { loadPreferenceCustodyV54SourceBundle, validatePreferenceCustodyManifestV55, validatePreferenceCustodyManifestV55Build } from './lib/preference-custody-manifest-v55.mjs';
const manifestPath = process.argv[2] ?? 'data/research/preference-custody/control-manifest-v55.json';
const buildPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v55.json';
...
let build;
try { build = load(buildPath); } catch (error) { console.error(`- Preference Custody v55 build could not be read: ${error.message}`); process.exit(1); }
...
const errors = validatePreferenceCustodyManifestV55Build(build, manifest, baseBuild, targetBuild, targetFixture, baseSources);
