...
import argparse, concurrent.futures, datetime as dt, hashlib, json, pathlib, ssl, sys, urllib.error, urllib.request
from urllib.parse import urlparse
ROOT=pathlib.Path(__file__).resolve().parents[3]
DATA=ROOT/'data/intake/status-sovereignty-rd-wave03-rd04-ca-sd-wa-successor-protocol'
...
def validate(p):
 assert p['schema_version']=='ssc-rd04-wave03-ca-sd-wa-successor-route-ledger@1'
 assert p['counts']['fixed_routes']==30 and len(p['routes'])==30
 assert p['execution_contract']['maximum_attempts_per_route']==1
 assert p['execution_contract']['result_spawned_requests']==0
 allowed=set(p['execution_contract']['allowed_final_hosts']); ids=set(); urls=set()
 for n,r in enumerate(p['routes'],1):
  assert r['route_ordinal']==n and r['route_id']==f'RD04-W03-T3-{n:03d}'
  assert r['route_id'] not in ids and r['url'] not in urls; ids.add(r['route_id']); urls.add(r['url'])
  u=urlparse(r['url']); assert u.scheme=='https' and (u.hostname or '').lower() in allowed
...
def capture(route,allowed,out):
 ...
 request={'route_id':rid,'url':route['url'],'method':'GET','maximum_attempts':1,'result_spawned_requests':0}
 (d/'request.json').write_text(dump(request))
 ...
 try:
  opener=urllib.request.build_opener(BoundRedirect(allowed))
  req=urllib.request.Request(route['url'],headers={'User-Agent':'SSC-RD04-W03-bounded-capture/1.0','Accept':'*/*'})
  try:
   with opener.open(req,timeout=45,context=ssl.create_default_context()) as resp:
 ...
 (d/'body.bin').write_bytes(body); (d/'headers.json').write_text(dump(headers))
 receipt={'route_id':rid,'route_ordinal':route['route_ordinal'],'state_scope':route['state_scope'],'route_category':route['route_category'],'target_cell_keys':route['target_cell_keys'],'state':state,'request_attempts':1,'started_at':start,'completed_at':dt.datetime.now(dt.timezone.utc).isoformat(),'http_status':status,'final_url':final_url,'final_host':(urlparse(final_url).hostname or '').lower() if final_url else None,'body_bytes':len(body),'body_sha256':sha(body),'headers_sha256':sha(dump(headers).encode()),'error':error,'source_admitted':False,'field_classification_effect':'none','row_terminalization_effect':'none','class_closure_effect':'none','result_spawned_requests':0,'outside_human_dependency':False}
 (d/'receipt.json').write_text(dump(receipt)); return receipt
def main():
 ...
 if args.validate_only:
  print('rd04_ca_sd_wa_successor_protocol_validation=pass'); print('fixed_routes=30'); print('target_cells=9'); print('result_spawned_requests=0'); return
 ...
 results.sort(key=lambda x:x['route_ordinal']); (out/'route-results.json').write_text(dump({'schema_version':'ssc-rd04-wave03-ca-sd-wa-successor-route-results@1','protocol_id':p['protocol_id'],'routes':results}))
 counts={k:sum(1 for r in results if r['state']==k) for k in ['http_success_pending_separate_adjudication','terminal_http_non_success','terminal_disallowed_final_host','terminal_transport_failure','terminal_body_limit_exceeded']}
 receipt={'schema_version':'ssc-rd04-wave03-ca-sd-wa-successor-execution-receipt@1','protocol_id':p['protocol_id'],'fixed_routes':30,'terminal_routes':30,'started_at':started,'completed_at':dt.datetime.now(dt.timezone.utc).isoformat(),'state_counts':counts,'source_admissions':0,'field_classifications':0,'row_terminalizations':0,'class_closures':0,'result_spawned_requests':0,'outside_human_dependency':False}; (out/'execution-receipt.json').write_text(dump(receipt))
 entries=[]
 ...
 (out/'manifest.json').write_text(dump({'schema_version':'ssc-rd04-wave03-ca-sd-wa-successor-capture-manifest@1','entries':entries,'combined_sha256':sha(''.join(f"{e['path']}\0{e['sha256']}\n" for e in entries).encode())}))
