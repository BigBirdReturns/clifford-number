...
export function buildStatusSovereignty() {
  ...
  const report = {
    ...
    counts: {
      ...
      field_source_records: waveSources.records.length,
  ...
  const fieldSourceRows = waveSources.records.map((row) => `<tr><td><code>${esc(row.source_id)}</code></td><td><a href="${esc(row.url)}">${esc(row.title)}</a><br>${esc(row.publisher)}</td><td>${esc(row.source_class)}</td><td>${esc(row.retrieval.status)}; exact source bytes not preserved</td></tr>`).join('');
  const waveRows = wave.observations.map((row) => `<tr><td><code>${esc(row.observation_id)}</code><br><code>${esc(row.lane_id)}</code></td><td>${esc(row.disposition)}</td><td>${esc(row.working_interpretation)}</td><td>${esc(row.review_state)}</td></tr>`).join('');
...
<h2>Current state</h2><pre>${esc(JSON.stringify(hypothesis.current_state, null, 2))}</pre><h2>Boundary</h2><pre class="boundary">${esc(JSON.stringify(hypothesis.boundaries, null, 2))}</pre><p class="small"><code>Wave 01 release SHA-256: ${waveRelease.combined_sha256}</code></p><p class="small"><code>SSC release SHA-256: ${manifest.combined_sha256}</code></p></body></html>`;
  ...
  return { hypothesis, fanout, sources, wave, waveSources, waveRelease, manifest, report };
}
