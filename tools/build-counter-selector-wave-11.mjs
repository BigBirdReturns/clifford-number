...
export const releaseScope=[
  '.github/workflows/counter-selector-wave-11.yml',
  'data/project/counter-selector-wave-11-batch-04.json',
  'data/project/counter-selector-wave-11-source-registry.json',
  'schemas/counter-selector-artifact-acquisition-b04.schema.json',
  'docs/methods/counter-selector-artifact-acquisition-b04.md',
  'docs/milestones/counter-selector-wave-11.md',
  'tools/build-counter-selector-wave-11.mjs',
  'tools/validate-counter-selector-wave-11.mjs',
  'test/counter-selector-wave-11.test.js'
];

export function manifest(){
  const entries=releaseScope.map(p=>{const b=bytes(p);return{path:p,sha256:sha256(b),bytes:b.length}});
  return {
    schema_version:'counter-selector-wave-11-release-manifest@1',
...
export function acquisition(contract,sources){
  ...
  return {
    schema_version:'counter-selector-artifact-acquisition-b04-registry@1',
    program_id:contract.program_id,
    wave_id:contract.wave_id,
    batch_id:contract.batch_id,
    as_of:contract.as_of,
    status:'batch_04_acquired_five_packets_ready_review_not_started',
...
export function packets(contract){
  return {
    schema_version:'counter-selector-blind-packet-registry-b04@1',
    program_id:contract.program_id,
    wave_id:contract.wave_id,
    batch_id:contract.batch_id,
    as_of:contract.as_of,
    status:'five_identity_minimized_packets_ready_blind_review_not_started',
    counts:{
    ...
    next_action:"Run procedurally separated blind-first review on the five Batch 04 packets. Preserve individual, collective, recommendation, merits-control, and correction-mechanism scope.",
    boundaries:{
...
export function report(contract,src,acq,pkt,m){
  return {
    schema_version:'counter-selector-wave-11-report@1',
    program_id:contract.program_id,
    wave_id:contract.wave_id,
    batch_id:contract.batch_id,
    as_of:contract.as_of,
    ...
    release_manifest:{
      path:'data/project/counter-selector-wave-11-release-manifest.json',
...
export function html(r){
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="robots" content="noindex,nofollow"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Counter-Selector Wave 11</title><style>body{font-family:system-ui;max-width:1200px;margin:auto;padding:36px;line-height:1.5;background:#f3f0e9;color:#181714}h1{font-size:clamp(2.5rem,6vw,5rem);line-height:.95}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.card,table{background:#fff;border:1px solid #c9c1b4;border-radius:12px}.card{padding:16px}.card b{display:block;font-size:2.2rem}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{padding:10px;border-bottom:1px solid #ddd;text-align:left}</style></head><body><p><b>CLIFFORD NUMBER · ${esc(r.wave_id)}</b></p><h1>Refusal, warnings, controls, and correction enter together</h1><div class="grid"><div class="card"><b>${r.counts.batch_objects}</b>objects</div><div class="card"><b>${r.counts.qualifying_acquisitions}</b>qualifying</div><div class="card"><b>${r.counts.partial_acquisitions}</b>partial</div><div class="card"><b>${r.counts.blind_reviews_executed}</b>reviews</div></div><table><thead><tr><th>Candidate</th><th>Class</th><th>Acquisition</th><th>Packet</th></tr></thead><tbody>${rows}</tbody></table><p><strong>Next:</strong> ${esc(r.next_action)}</p><p><code>${esc(r.release_manifest.combined_sha256)}</code></p></body></html>\n`
}

export function build(){
  const c=read('data/project/counter-selector-wave-11-batch-04.json');
  const s=read('data/project/counter-selector-wave-11-source-registry.json');
  const a=acquisition(c,s),p=packets(c);
  writeJson('data/project/counter-selector-artifact-acquisition-b04-registry.json',a);
  writeJson('data/project/counter-selector-blind-packet-registry-b04.json',p);
  const m=manifest();
  writeJson('data/project/counter-selector-wave-11-release-manifest.json',m);
  const r=report(c,s,a,p,m);
  writeJson('reports/core-thesis/counter-selector-wave-11/data.json',r);
  writeText('reports/core-thesis/counter-selector-wave-11/index.html',html(r));
  console.log(`build-counter-selector-wave-11: ${a.counts.batch_objects} objects, ${a.counts.qualifying_acquisitions} qualifying, ${p.counts.packets_ready} packets`)
}
