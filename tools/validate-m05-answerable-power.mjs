...
if(registry.stories.length!==15)fail(`expected 15 stories, got ${registry.stories.length}`);
if(fanout.lanes.length!==18)fail(`expected 18 lanes, got ${fanout.lanes.length}`);
...
const expectedCounts={standalone_actor:3,exact_overlap:3,constitutional_mechanism:5,answer_story:3,non_link:1};
...
if(report.counts.stories!==15 || report.counts.lanes!==18)fail('report counts drift');
...
if(!registry.stories.some((row)=>row.story_id==='M05-S15'))fail('M05-S15 missing');
if(!fanout.lanes.some((row)=>row.lane_id==='A18'&&row.story_id==='M05-S15'))fail('A18 missing or disconnected');
if(registry.boundaries.steel_mirror_differential_proves_bad_faith!==false || fanout.boundaries.steel_mirror_differential_proves_bad_faith!==false)fail('Steel Mirror bad-faith boundary drift');
