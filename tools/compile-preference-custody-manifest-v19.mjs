...
import {
  compilePreferenceCustodyManifestV19,
  renderPreferenceCustodyManifestV19Markdown
} from './lib/preference-custody-manifest-v19.mjs';

const manifestPath = process.argv[2] ?? 'data/research/preference-custody/control-manifest-v19.json';
const jsonPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v19.json';
const markdownPath = process.argv[4] ?? 'build/research/preference-custody-laboratory-floor-v19.md';
...
const choiceBuild = compilePreferenceChoiceEffectivenessFixture(readJson('data/research/preference-custody/choice-effectiveness.fixture.json'));
const compiled = compilePreferenceCustodyManifestV19(readJson(manifestPath), baseV18, choiceBuild);
mkdirSync(dirname(jsonPath), { recursive: true });
mkdirSync(dirname(markdownPath), { recursive: true });
writeFileSync(jsonPath, JSON.stringify(compiled, null, 2) + '\n');
writeFileSync(markdownPath, renderPreferenceCustodyManifestV19Markdown(compiled));
