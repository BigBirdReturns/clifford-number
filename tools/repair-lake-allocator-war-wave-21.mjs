...
function replaceRequired(source, before, after, label, expected = 1) {
  const count = source.split(before).length - 1;
  if (count !== expected) throw new Error(`${label}: expected ${expected} marker(s), saw ${count}`);
...
policy.projection_contract = {
  contract_name: 'wave21_source_registry_and_aggregate_projection_views',
  target_identifier_keys: [
    ...
    'allocator_record_id',
    'allocator_class_id',
    'allocator_estate_feed_id',
    'allocator_program_feed_id'
  ],
  allowed_generated_paths: [
    policy.paths.projection,
    policy.paths.reconciliation
  ],
  divergence_classification: 'typed_wave21_source_projection_and_reconciliation_views',
  divergence_disposition: 'retain_declared_typed_views',
  source_only_classification: 'declared_wave21_semantic_or_routing_identifier',
  source_only_disposition: 'retain_declared_source_identifier',
  projection_hash_equality_required: false,
  cross_key_join_authorized: false,
  graph_effect: 'none'
};
policy.decision_law.typed_projection_views_may_differ_by_role = true;
policy.boundaries.typed_projection_view_proves_identity_or_truth = false;
policy.boundaries.typed_projection_view_authorizes_cross_key_join = false;
writeJson(policyPath, policy);

const builderPath = 'tools/build-lake-allocator-war-wave-21.mjs';
let builder = read(builderPath);
for (const [schema, idMarker, expected] of [
  ['lake-allocator-war-observation-wave-21@1', 'allocator_record_id:', 2],
  ['lake-allocator-war-waterline-class-wave-21@1', 'allocator_class_id:', 2],
  ['lake-allocator-war-estate-acquisition-wave-21@1', 'allocator_estate_feed_id:', 1],
  ['lake-allocator-war-program-feed-wave-21@1', 'allocator_program_feed_id:', 1]
]) {
  const before = `    schema_version: '${schema}',\n    ${idMarker}`;
  const after = `    schema_version: '${schema}',\n    program_id: policy.program_id,\n    wave_id: policy.wave_id,\n    ${idMarker}`;
  if (!builder.includes(`schema_version: '${schema}',\n    program_id: policy.program_id,`)) {
    builder = replaceRequired(builder, before, after, `${schema} program identity`, expected);
  }
}
builder = builder.replace("  receipt_key: 'LAW-W21-RECEIPT',\n", '');
if (!builder.includes("schema_version: 'lake-allocator-war-wave-21-receipt@1',\n  program_id: policy.program_id,")) {
...
if (!builder.includes('  projection_contract: policy.projection_contract,\n  source_registry_digests:')) {
  builder = replaceRequired(
    builder,
...
builder = builder.replace("  projection_key: 'LAW-W21-PROJECTION',\n", '');
if (!builder.includes("schema_version: 'lake-allocator-war-wave-21@1',\n  program_id: policy.program_id,")) {
  builder = replaceRequired(
    builder,
...
reconciler = reconciler.replace("  reconciliation_key: 'LAW-W21-RECONCILIATION',\n", '');
if (!reconciler.includes("schema_version: 'lake-allocator-war-wave-21-reconciliation@1',\n  program_id: policy.program_id,")) {
...
if (!reconciler.includes('  projection_contract: policy.projection_contract,\n  source_registry_digests:')) {
  reconciler = replaceRequired(
    reconciler,
    '  as_of: policy.as_of,\n  source_registry_digests:',
    '  as_of: policy.as_of,\n  projection_contract: policy.projection_contract,\n  source_registry_digests:',
    'reconciliation projection contract'
  );
}
write(reconcilerPath, reconciler);

const stabilizerPath = 'tools/stabilize-lake-allocator-war-wave-21.mjs';
write(stabilizerPath, `#!/usr/bin/env node
import fs from 'node:fs';
...
if (!validator.includes("if (policy.projection_contract.graph_effect !== 'none')")) {
  validator = replaceRequired(
    ...
    'validator projection contract'
  );
}
if (!validator.includes("row.program_id !== policy.program_id")) {
  validator = replaceRequired(
    validator,
    ...
    'validator row identity'
  );
}
if (!validator.includes("reconciliation.program_id !== policy.program_id")) {
  validator = replaceRequired(
    validator,
    "  if (reconciliation) {\n    if (!reconciliation.current_state?.source_projection_index_complete)",
    ...
    'validator reconciliation identity'
  );
}
...
if (!tests.includes('drift row program identity')) {
  tests = replaceRequired(
    ...
    'adversarial projection contract mutations'
  );
}
write(testPath, tests);

console.log('allocator-war Wave 21 identifier and projection contracts repaired');
