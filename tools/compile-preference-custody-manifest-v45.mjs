...
import {
  ...
  renderPreferenceCustodyManifestV45Markdown
...
execFileSync(process.execPath, ['tools/compile-preference-linkage-event-estimand-scope-interpretation-assurance.mjs'], { stdio: 'inherit' });

const manifestPath = process.argv[2] ?? 'data/research/preference-custody/control-manifest-v45.json';
const jsonPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v45.json';
const markdownPath = process.argv[4] ?? 'build/research/preference-custody-laboratory-floor-v45.md';
...
const compiled = compilePreferenceCustodyManifestV45(manifest, baseBuild, targetBuild, targetFixture, baseSources);
mkdirSync(dirname(jsonPath), { recursive: true });
mkdirSync(dirname(markdownPath), { recursive: true });
writeFileSync(jsonPath, JSON.stringify(compiled, null, 2) + '\n');
writeFileSync(markdownPath, renderPreferenceCustodyManifestV45Markdown(compiled));
