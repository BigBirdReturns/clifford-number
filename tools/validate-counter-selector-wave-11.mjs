...
import { acquisition, packets, manifest, report, html } from './build-counter-selector-wave-11.mjs';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');
const read=rel=>JSON.parse(fs.readFileSync(path.join(root,rel),'utf8'));
const text=rel=>fs.readFileSync(path.join(root,rel),'utf8');

export function validateContractObject(c,s){
  assert.equal(c.schema_version,'counter-selector-wave-11-program@1');
  assert.equal(c.wave_id,'CS-W11-B04');
  assert.equal(c.batch_id,'CS-AQ-B04');
...
export function validate(){
  ...
  const a=read('data/project/counter-selector-artifact-acquisition-b04-registry.json');
  const p=read('data/project/counter-selector-blind-packet-registry-b04.json');
  const m=read('data/project/counter-selector-wave-11-release-manifest.json');
  const r=read('reports/core-thesis/counter-selector-wave-11/data.json');
  const h=text('reports/core-thesis/counter-selector-wave-11/index.html');
  ...
  console.log(`validate-counter-selector-wave-11: PASS (${c.records.length} objects, ${p.packets.length} packets, ${m.combined_sha256})`);
  return true;
}
