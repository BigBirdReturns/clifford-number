...
import { validateSchoolhouseNcStaticNonprofitAnchorCustody } from './validate-schoolhouse-nc-static-nonprofit-anchor-custody.mjs';
...
export function validateBVVCDefenseCapital(dir = DEFAULT_DIR) {
...

  {
    ...
    check(manifest.counts.source_inventory_rows === 482, 'second-level source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'second-level coverage-denominator count drift');
...

  {
    ...
    check(manifest.counts.source_inventory_rows === 482, 'final residual source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'final residual coverage-denominator count drift');
...

  {
    ...
    check(manifest.counts.source_inventory_rows === 482, 'complete-PDF source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'complete-PDF coverage-denominator count drift');
...

  {
    ...
    check(manifest.counts.source_inventory_rows === 482, 'first-party source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'first-party coverage-denominator count drift');
...

  {
    ...
    check(manifest.counts.source_inventory_rows === 482, 'archive source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'archive coverage-denominator count drift');
...

  {
    ...
    check(manifest.counts.source_inventory_rows === 482, 'launch-era source denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'launch-era coverage denominator drift');
    ...
    check(launchCustody.acquisitions.baseline.workflow_run_id === 31024272835 && launchCustody.acquisitions.baseline.artifact_id === 8937954996 && launchCustody.acquisitions.baseline.artifact_digest === 'sha256:e9d4346b9ff2831cbe0b7a2e3cae3aaebcc654f6af0742d80534904d9ca3b8b1' && launchCustody.acquisitions.baseline.route_results_sha256 === 'aa55078fbbd5feeeb3c205eba6facf797c7c3f9862285345a175af0f5e94ea58', 'launch-era baseline custody drift');
    check(launchCustody.acquisitions.bounded_failed_snapshot_replay.workflow_run_id === 31026429458 && launchCustody.acquisitions.bounded_failed_snapshot_replay.artifact_id === 8938827444 && launchCustody.acquisitions.bounded_failed_snapshot_replay.artifact_digest === 'sha256:3fcc4191cf2e6aa3f84eadc5b5265c4e9af13a58a7391b2407a194d6d147a6c9' && launchCustody.acquisitions.bounded_failed_snapshot_replay.route_results_sha256 === '0e51575f0a126a897a5b90f78263babae535365d3f234ef136ad68329fe53725', 'launch-era replay custody drift');
  ...
  {
    const reconciliationCustody = readJson(path.join(dir, 'schoolhouse-archive-minimum-legal-surface-reconciliation-custody.json'));
    const reconciliationAttempts = readJsonl(path.join(dir, 'schoolhouse-archive-minimum-legal-surface-attempt-results.jsonl'));
    const reconciliationRoutes = readJsonl(path.join(dir, 'schoolhouse-archive-minimum-legal-surface-route-reconciliation.jsonl'));
    const reconciliationSignals = readJsonl(path.join(dir, 'schoolhouse-archive-minimum-legal-surface-legal-signals.jsonl'));
    const reconciliationLinks = readJsonl(path.join(dir, 'schoolhouse-archive-minimum-legal-surface-link-inventory.jsonl'));
    ...
    check(manifest.counts.source_inventory_rows === 482, 'archive reconciliation source denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'archive reconciliation coverage denominator drift');
    ...
    check(reconciliationCustody.canonical_parent.commit === '170cffa6c2a0c1e6fcd5124d713f054d228b4979' && reconciliationCustody.canonical_parent.tree === 'a991c88abe08aeb11687a22de1cb584431dfb99b', 'archive reconciliation parent custody drift');
    check(reconciliationCustody.acquisitions.minimum_surface_baseline.workflow_run_id === 31030908630 && reconciliationCustody.acquisitions.minimum_surface_baseline.artifact_id === 8940650245 && reconciliationCustody.acquisitions.minimum_surface_baseline.artifact_digest === 'sha256:dc499ff1899811f84be3873df2787835dd70f95a73a64d7652ede4d51f48f7da' && reconciliationCustody.acquisitions.minimum_surface_baseline.route_results_sha256 === 'e951990ff347b4ad08f6edfeb52189244b7d58d8a852251a1cb2bbe4f13c83b6', 'archive reconciliation baseline acquisition drift');
    check(reconciliationCustody.acquisitions.bounded_transport_replay.workflow_run_id === 31032164683 && reconciliationCustody.acquisitions.bounded_transport_replay.artifact_id === 8941112372 && reconciliationCustody.acquisitions.bounded_transport_replay.artifact_digest === 'sha256:ae34561d531c077e56473553b09fc2fe834c32d23f03d7eef3f422c700daf831' && reconciliationCustody.acquisitions.bounded_transport_replay.route_results_sha256 === '38355d7ed95c3aec373c0eb9174d71b158318b6068a258abd76a70cd1f5667d2', 'archive reconciliation replay acquisition drift');
  ...
  if (fs.existsSync(path.join(dir, 'schoolhouse-fl-charity-exact-query-custody.json'))) {
    const flCharityCustody = readJson(path.join(dir, 'schoolhouse-fl-charity-exact-query-custody.json'));
    const flCharityExecutions = readJsonl(path.join(dir, 'schoolhouse-fl-charity-exact-query-executions.jsonl'));
    const flCharityAttempts = readJsonl(path.join(dir, 'schoolhouse-fl-charity-exact-query-attempt-results.jsonl'));
    const flCharitySourceRows = readJsonl(path.join(dir, 'source-inventory-17.jsonl'));
    const receiptIds = new Set(sourceInventory.map(row => row.receipt_id));
    ...
    check(manifest.counts.source_inventory_rows === 482, 'Florida charity source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31, 'Florida charity coverage-denominator drift');
    check(manifest.storage_contract.source_inventory_parts.at(-5) === 'source-inventory-17.jsonl' && manifest.storage_contract.source_inventory_parts.at(-4) === 'source-inventory-18.jsonl' && manifest.storage_contract.source_inventory_parts.at(-3) === 'source-inventory-19.jsonl' && manifest.storage_contract.source_inventory_parts.at(-2) === 'source-inventory-20.jsonl' && manifest.storage_contract.source_inventory_parts.at(-1) === 'source-inventory-21.jsonl', 'Florida charity and Form 990 source-inventory tail drift');
    ...
    check(flCharityCustody.canonical_parent.commit === '1f04c0abbf89f1529dfd428389392b3425499bc3' && flCharityCustody.canonical_parent.tree === '33e540e995455dfd28ebe1cd6a7f09c29844d461', 'Florida charity parent custody drift');
  ...
  if (fs.existsSync(path.join(dir, 'schoolhouse-fl-corporate-identity-census-custody.json'))) {
    const corporateCustody = readJson(path.join(dir, 'schoolhouse-fl-corporate-identity-census-custody.json'));
    const corporateSource = readJson(path.join(dir, 'schoolhouse-fl-corporate-identity-census-source-receipt.json'));
    const corporateIndex = readJson(path.join(dir, 'schoolhouse-fl-corporate-identity-census-remote-zip-index.json'));
    const corporateRanges = readJsonl(path.join(dir, 'schoolhouse-fl-corporate-identity-census-range-request-receipts.jsonl'));
    const corporateMembers = readJsonl(path.join(dir, 'schoolhouse-fl-corporate-identity-census-member-receipts.jsonl'));
    const corporateTargets = readJson(path.join(dir, 'schoolhouse-fl-corporate-identity-census-target-matrix.json'));
    const corporateCandidates = readJsonl(path.join(dir, 'schoolhouse-fl-corporate-identity-census-candidate-records.jsonl'));
    const corporateAdjudication = readJson(path.join(dir, 'schoolhouse-fl-corporate-identity-census-adjudication.json'));
    const corporateSourceRows = readJsonl(path.join(dir, 'source-inventory-19.jsonl'));
    ...
    check(manifest.counts.source_inventory_rows === 482 && sourceInventory.length === 482, 'complete Florida corporate source-inventory denominator drift');
    check(manifest.counts.coverage_denominator_rows === 31 && coverage.denominators.length === 31, 'complete Florida corporate coverage denominator drift');
    ...
    check(manifest.storage_contract.source_inventory_parts.at(-3) === 'source-inventory-19.jsonl' && manifest.storage_contract.source_inventory_parts.at(-2) === 'source-inventory-20.jsonl' && manifest.storage_contract.source_inventory_parts.at(-1) === 'source-inventory-21.jsonl', 'complete Florida corporate source-inventory tail drift');
    check(manifest.source_inventory.evidence_class_counts.official === 257, 'complete Florida corporate official evidence count drift');
    ...
    check(corporateCustody.canonical_parent.commit === 'e46d4aa98955e890a1bd2820dc43c52bc256490e' && corporateCustody.canonical_parent.tree === 'f6110927bb6a0241169f1e915db732e41fcb52cf', 'complete Florida corporate parent custody drift');
    check(corporateCustody.acquisitions.authenticated_complete_census.workflow_run_id === 31056239350 && corporateCustody.acquisitions.authenticated_complete_census.artifact_id === 8950526654 && corporateCustody.acquisitions.authenticated_complete_census.artifact_digest === 'sha256:98925c7a750f23cf8ed10fb1123ed0d79d78dcca79d4c67a2b0e26ed86146746', 'complete Florida corporate acquisition custody drift');
  ...
  for (const error of validateSchoolhouseNcStaticNonprofitAnchorCustody(dir)) errors.push(`School.House NC static nonprofit anchors: ${error}`);
