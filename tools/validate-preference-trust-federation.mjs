import { readFileSync } from 'node:fs';
import {
  validatePreferenceTrustFederationBuild,
  validatePreferenceTrustFederationFixture
} from './lib/preference-trust-federation.mjs';

const sourcePath = process.argv[2] ?? 'data/research/preference-custody/trust-federation.fixture.json';
const buildPath = process.argv[3] ?? 'build/research/preference-trust-federation.json';

const fixture = JSON.parse(readFileSync(sourcePath, 'utf8'));
const compiled = JSON.parse(readFileSync(buildPath, 'utf8'));
const errors = [
  ...validatePreferenceTrustFederationFixture(fixture).map(error => `source: ${error}`),
  ...validatePreferenceTrustFederationBuild(compiled).map(error => `build: ${error}`)
];

if (errors.length) {
  console.error(errors.join('\n'));
  process.exitCode = 1;
} else {
  console.log('Cross-organizational trust federation, revocation, and recovery custody: PASS');
}
