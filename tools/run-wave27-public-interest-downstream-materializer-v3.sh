...
self='tools/run-wave27-public-interest-downstream-materializer-v3.sh'
...
replacement = '''function insertIntoNamedExactArray(text, basinId, values) {
...
PY
rm -f "$self" 'tools/run-wave27-public-interest-downstream-materializer-v2.sh'
exec bash "$runner"
