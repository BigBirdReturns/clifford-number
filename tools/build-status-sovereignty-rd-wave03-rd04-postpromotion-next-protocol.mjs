...
export function buildCoreObjects() {
  ...
  const routeQueryContract = {
    ...
    issue: 1017,
    fixed_route_count: normalizedRoutes.length,
    maximum_logical_route_attempts: normalizedRoutes.length,
    maximum_total_requests: normalizedRoutes.length,
    maximum_physical_requests: normalizedRoutes.length,
    maximum_total_request_semantics: 'physical_http_requests_including_redirects',
    maximum_attempts_per_route: 1,
    maximum_redirects_per_route: 4,
    redirects_consume_total_request_budget: true,
    ...
    runner_root_override_allowed: false,
    protocol_input_sha256_binding: true,
    result_spawned_requests: 0,
    receipt_preimage_field: 'receipt_preimage_sha256',
    final_receipt_ledger_path: 'receipt-file-ledger.json',
    final_receipt_sha256_embedded: false,
    ...
    retained_transport_fields: [
      ...
      'content_type', 'body_bytes', 'body_sha256', 'physical_request_count',
      'receipt_path', 'receipt_preimage_sha256',
    ],
    terminal_states: [
  ...
  const summary = {
    ...
    adoption_effect: 'none',
    graph_effect: 'none',
    captured_transport_logical_attempts: 5,
    captured_transport_physical_requests: 5,
    captured_transport_artifact_id: 9018149945,
    captured_transport_artifact_sha256: '9b828cef364b156cc995409e445d5a1250c981127d21b841d56b05ef14b036e3',
    additional_execution_authorized: false,
    next_operation: 'adjudicate the retained five-route transport artifact offline; no additional route execution is authorized',
  };
...
function buildManifest(coreObjects) {
  ...
  return {
    ...
    schema_version: SCHEMA_VERSION,
    canonical_parent: 'a18f19ddc923e553848a5165cdf29fb1b9add97a',
    product_branch: 'agent/ssc-rd04-postpromotion-execution-control-repair-v1',
