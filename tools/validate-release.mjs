...
// Temporal identity layer (canonical AXM Genesis ids). The artifact must be a
// deterministic function of the ledger — recompute it and require exact
// agreement — and must carry its reconciliation statement citing the shared
// vectors and the conformance test, so a stale or hand-edited artifact, or one
// silently stripped or weakened of that statement, fails the release.
{
  ...
  assert(identity.scheme?.status === 'reconciled', `axm-identity scheme.status must be "reconciled" (byte-for-byte against axm-genesis), got ${JSON.stringify(identity.scheme?.status)}`);
  // Guard the reconciliation statement against being stripped or weakened: it
  // must still cite the shared vectors (by hash) and the conformance test.
  assert(identity.scheme?.vectors?.sha256 === EXPECTED_VECTOR_SHA, `axm-identity scheme must cite the shared vectors sha256 ${EXPECTED_VECTOR_SHA}`);
  assert(typeof identity.scheme?.serialization === 'string'
    ...
    'axm-identity scheme.serialization must document reconciliation against axm-genesis axm_verify.identity, the shared vectors, and the conformance test');
