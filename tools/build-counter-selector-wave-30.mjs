...
export const STATIC_MANIFEST_PATHS = [
  '.github/workflows/counter-selector-wave-30.yml',
  SOURCE_PATH,
  'schemas/counter-selector-handoff-controls.schema.json',
  'docs/methods/counter-selector-public-handoff-controls.md',
  'docs/milestones/counter-selector-wave-30.md',
  'tools/build-counter-selector-wave-30.mjs',
  'tools/validate-counter-selector-wave-30.mjs',
  'test/counter-selector-wave-30.test.js'
];
...
export function deriveRegistry(source) {
  return {
    schema_version: 'counter-selector-wave-30-handoff-registry@1',
...
export function deriveManifest(source) {
  ...
  return {
    schema_version: 'counter-selector-wave-30-release-manifest@1',
    program_id: source.program_id,
    wave_id: source.wave_id,
    as_of: source.as_of,
...
export function deriveReport(source, registry, manifest) {
  return {
    schema_version: 'counter-selector-wave-30-report@1',
    program_id: source.program_id,
    wave_id: source.wave_id,
    parent_wave_id: source.parent_wave_id,
    as_of: source.as_of,
...
<td>${escapeHtml(control.components.authority_or_credential_transfer)}</td>
<td>${escapeHtml(control.components.successor_action)}</td>
</tr>`).join('');
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="robots" content="noindex,nofollow"><title>Counter-Selector Wave 30</title>
...
<body><p><strong>CLIFFORD NUMBER · COUNTER-SELECTOR · CS-W30-HC-01</strong></p>
<h1>Transfer authority without inventing a state package.</h1>
<p class="state">TWO BOUNDED AUTHORITY TRANSFERS · ZERO COMPLETE HANDOFFS · ZERO OPERATORS</p>
<div class="grid"><article class="card"><b>${report.counts.handoff_controls_audited}</b>handoff controls</article>
<article class="card"><b>${report.counts.authority_transfer_events}</b>authority transfers</article>
<article class="card"><b>${report.counts.successor_action_chains}</b>successor action chains</article>
<article class="card"><b>${report.counts.complete_direct_handoffs}</b>complete handoffs</article></div>
<table><thead><tr><th>ID</th><th>Transition</th><th>Classification</th><th>Authority</th><th>Successor action</th></tr></thead><tbody>${rows}</tbody></table>
<div class="boundary"><strong>Authority ceiling</strong><pre>role change ≠ complete state handoff
key rotation ≠ knowledge transfer
successor action ≠ predecessor state package
continuous operation ≠ complete custody
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(`build-counter-selector-wave-30: ${source.counts.handoff_controls_audited} controls, ${source.counts.complete_direct_handoffs} complete handoffs`);
}
