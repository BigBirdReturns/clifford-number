...
// The endpoint release ships the current surface/hop/receipt datasets plus the separately-declared legacy context graph, keyed by
...
const prelude = `globalThis.CLIFFORD_GRAPH_CONFIG = globalThis.CLIFFORD_GRAPH_CONFIG ?? {
  ...
  // (legacy: graph.json, hops: hop-graph, surfaces: surface-graph, receipts: receipt-graph).
  // An empty value uses the embedded demo copy of the published corpus.
  endpoints: { legacy: '', hops: '', surfaces: '', receipts: '' },
  demoFallback: true
};
