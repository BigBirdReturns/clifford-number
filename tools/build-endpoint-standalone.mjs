...
const datasets = {
  ...
  boundary: readJson('build/public-data-boundary.json')
};
...
const prelude = `globalThis.CLIFFORD_GRAPH_CONFIG = globalThis.CLIFFORD_GRAPH_CONFIG ?? {
  ...
  // (research, hops, surfaces, receipts, and the public projection-boundary contract).
  // An empty value uses the embedded demo copy of the published corpus.
  endpoints: { research: '', hops: '', surfaces: '', receipts: '', boundary: '' },
  demoFallback: true
};
