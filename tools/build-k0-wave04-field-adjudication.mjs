...
export const fieldScope = [
  '.github/workflows/k0-wave04-field-adjudication.yml',
  'data/research/k0-wave04-field-adjudication.json',
  'docs/milestones/m05-k0-wave04-field-adjudication.md',
  'tools/build-k0-wave04-field-adjudication.mjs',
  'tools/validate-k0-wave04-field-adjudication.mjs',
  'test/k0-wave04-field-adjudication.test.js'
];

export function computeWave04FieldManifest() {
  const entries = fieldScope.map(rel => {
    const data = bytes(rel);
    return { path: rel, sha256: sha256(data), bytes: data.length };
  });
  return {
    schema_version: 'k0-wave04-field-adjudication-release-manifest@1',
    program_id: 'M-05',
    layer_id: 'K0',
    audit_id: 'K0-W04-FIELD-2026-07-28-MAINTAINER',
    as_of: '2026-07-28',
    ...
    boundaries: {
      ...
      committee_reset_proves_capture: false,
      categorical_exclusion_proves_knower_reclassification: false,
      post_action_correction_completes_pre_action_chain: false,
      manifest_creates_graph_effect: false
    }
  };
}

const audit = read('data/research/k0-wave04-field-adjudication.json');
const wave = read('data/research/k0-role-neutral-wave-04.json');
const manifest = computeWave04FieldManifest();
...
const report = {
  schema_version: 'k0-wave04-field-adjudication-report@1',
  program_id: 'M-05',
  layer_id: 'K0',
  title: 'K0 Wave 04 advisory-committee field adjudication',
  ...
  counts: {
    ...
    seed_overlap_controls: dispositionCounts.seed_overlap_control_retained || 0,
  ...
  rows: audit.rows,
  current_result: {
    every_wave04_record_reviewed: audit.rows.length === wave.records.length,
  ...
  release_manifest: {
    path: 'data/project/k0-wave04-field-adjudication-release-manifest.json',
    combined_sha256: manifest.combined_sha256
  },
  boundaries: audit.boundaries
};

write('data/project/k0-wave04-field-adjudication-release-manifest.json', stable(manifest));
write('reports/core-thesis/answerable-power/k0-wave04-field-adjudication.json', stable(report));
...
const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>K0 Wave 04 field adjudication</title><style>body{font:16px/1.55 system-ui;max-width:1500px;margin:36px auto;padding:0 22px;background:#ece9df;color:#171717}code,pre{font-family:ui-monospace,SFMono-Regular,monospace}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.metric,table,pre{background:#fffdf7;border:1px solid #c9c1b2;border-radius:12px}.metric,pre{padding:15px}.metric b{display:block;font-size:1.8rem}table{border-collapse:collapse;width:100%}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left;vertical-align:top}.state{font-weight:800;color:#a43a00}</style></head><body><p><b>CLIFFORD NUMBER · M-05 · K0</b></p><h1>K0 Wave 04 advisory-committee field adjudication</h1><p class="state">MAINTAINER REVIEW ONLY · NO INCLUDED EVENTS · PUBLICATION BLOCKED · GRAPH INERT</p><div class="metrics"><div class="metric"><b>${report.counts.records_reviewed}</b>records reviewed</div><div class="metric"><b>${report.counts.bounded_non_link}</b>bounded non-links</div><div class="metric"><b>${report.counts.retained_candidate_only}</b>candidate only</div><div class="metric"><b>${report.counts.counterpower_controls}</b>counterpower controls</div><div class="metric"><b>${report.counts.included_events}</b>included events</div></div><h2>Field results</h2><table><tr><th>Record</th><th>Matter</th><th>Role</th><th>Disposition</th><th>Chain</th><th>Furthest</th><th>Open requirements / control finding</th></tr>${rows}</table><h2>Current result</h2><pre>${esc(JSON.stringify(report.current_result, null, 2))}</pre><h2>Boundary</h2><pre>${esc(JSON.stringify(report.boundaries, null, 2))}</pre><p><code>release SHA-256: ${manifest.combined_sha256}</code></p></body></html>`;
write('reports/core-thesis/answerable-power/k0-wave04-field-adjudication.html', html + '\n');

console.log(`build-k0-wave04-field-adjudication: ${audit.rows.length} records, ${report.counts.bounded_non_link} bounded non-link, ${report.counts.retained_candidate_only} candidate-only, 0 included events`);
