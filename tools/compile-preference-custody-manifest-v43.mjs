...
import { compilePreferenceCustodyManifestV43, renderPreferenceCustodyManifestV43Markdown } from './lib/preference-custody-manifest-v43.mjs';
execFileSync(process.execPath, ['tools/compile-preference-custody-manifest-v42.mjs'], { stdio: 'inherit' });
execFileSync(process.execPath, ['tools/compile-preference-linkage-interval-construction-assurance.mjs'], { stdio: 'inherit' });
const manifestPath = process.argv[2] ?? 'data/research/preference-custody/control-manifest-v43.json';
const jsonPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v43.json';
const markdownPath = process.argv[4] ?? 'build/research/preference-custody-laboratory-floor-v43.md';
...
const compiled = compilePreferenceCustodyManifestV43(manifest, baseBuild, intervalBuild, intervalFixture, baseSources);
mkdirSync(dirname(jsonPath), { recursive: true }); mkdirSync(dirname(markdownPath), { recursive: true });
writeFileSync(jsonPath, JSON.stringify(compiled, null, 2) + '\n'); writeFileSync(markdownPath, renderPreferenceCustodyManifestV43Markdown(compiled));
