...
export function validateSchemaContract(schema) {
  ...
  ok(schema?.properties?.authority?.const === 'two_wave03_terminal_class_receipts_promoted_without_cross_lane_empirical_authority', 'schema authority contract changed');
  ok(schema?.properties?.promoted_class_receipts?.minItems === 8 && schema?.properties?.promoted_class_receipts?.maxItems === 8, 'schema promoted-receipt denominator changed');
  ok(schema?.properties?.selected_classes_open?.minItems === 4 && schema?.properties?.selected_classes_open?.maxItems === 4, 'schema open-selected denominator changed');
  ok(schema?.properties?.counts?.properties?.canonical_residual_classes?.const === 42, 'schema canonical denominator changed');
  ok(schema?.properties?.counts?.properties?.closed_residual_classes?.const === 8, 'schema closed count changed');
  ok(schema?.properties?.counts?.properties?.open_residual_classes?.const === 34, 'schema open count changed');
  ok(schema?.properties?.counts?.properties?.wave_03_terminal_class_receipts?.const === 2, 'schema Wave-03 receipt count changed');
  ...
  for (const key of [
    ...
    'eight_closures_are_complete_compact',
...
export function validateValue(value, root = ROOT) {
  ...
  ok(value?.authority === 'two_wave03_terminal_class_receipts_promoted_without_cross_lane_empirical_authority', 'current-ledger authority changed');
  ok(value?.promoted_class_receipts?.length === 8, 'eight promoted receipts required');
  same(value.promoted_class_receipts.map((row) => row.class_id), [...CLOSED_IDS], 'promoted receipt order changed');
  ok(new Set(value.promoted_class_receipts.map((row) => row.class_id)).size === 8, 'promoted class IDs must be unique');
  ...
  ok(new Set(value.selected_classes_open.map((row) => row.class_id)).size === 4, 'open selected class IDs must be unique');
  ...
  ok(value.counts.wave_03_terminal_class_receipts === 2 && value.counts.classes_closed_this_wave === 2, 'Wave-03 terminal receipt accounting changed');
  ok(value.counts.closed_residual_classes === 8 && value.counts.open_residual_classes === 34, 'residual arithmetic changed');
  ...
  ok(value.current_result.classes_closed === 8 && value.current_result.classes_open === 34, 'current-result arithmetic changed');
  ok(value.current_result.wave_03_selected_attempts_terminal === 2, 'current-result Wave-03 terminal count changed');
...
export function validateRepository(root = ROOT) {
  ...
  for (const merge of [WAVE02_PROMOTION_MERGE, RD01_MERGE, RD03_MERGE]) {
...
function run() {
  ...
  console.log('Wave-03 current ledger validated: 34 open / 8 closed; RD-01-C06 and RD-03-C05 exact merge custody preserved');
}
