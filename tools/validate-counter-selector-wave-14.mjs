...
import { computeReleaseManifest, deriveRegistry, deriveDisagreementLedger, deriveReport, renderHtml } from './build-counter-selector-wave-14.mjs';
...
export function validateRegistryObject(registry, contract, parentRegistry) {
  assert.equal(registry.schema_version, 'counter-selector-blind-review-b05-registry@1');
  ...
  const expectedIds = ["CS-BLIND-0005", "CS-BLIND-0010", "CS-BLIND-0025", "CS-BLIND-0030"];
...
export function validateWave14() {
  const contract = read('data/project/counter-selector-wave-14-blind-review.json');
  const parentRegistry = read('data/project/counter-selector-blind-packet-registry-b05.json');
  const registry = read('data/project/counter-selector-blind-review-b05-registry.json');
  const ledger = read('data/project/counter-selector-review-disagreement-b05-ledger.json');
  const manifest = read('data/project/counter-selector-wave-14-release-manifest.json');
  const report = read('reports/core-thesis/counter-selector-wave-14/data.json');
  const html = readText('reports/core-thesis/counter-selector-wave-14/index.html');
  ...
  console.log(`validate-counter-selector-wave-14: PASS (${registry.packet_results.length} packets, ${registry.counts.bounded_dimension_supports} supports, ${manifest.combined_sha256})`);
  return true;
}

if (process.argv[1] === fileURLToPath(import.meta.url)) validateWave14();
