...
export const PREFERENCE_CUSTODY_MANIFEST_V31_SCHEMA_VERSION =
  'preference-custody-control-manifest-v31@1';
export const PREFERENCE_CUSTODY_MANIFEST_V31_BUILD_SCHEMA_VERSION =
  'preference-custody-control-manifest-v31-build@1';

const REQUIRED_CONTROL_IDS = Array.from({ length: 33 }, (_, index) =>
...
const REQUIRED_PRESERVED_FRONTIERS = [
  ...
  'cross_organizational_vendor_customer_and_regulator_succession',
  ...
  'federated_multilevel_and_successor_authority',
  'harm_reference_causality_measurement_and_compensation_objective_governance',
  'intersectional_and_unobserved_group_custody',
  'intertemporal_repeated_bargaining_and_commitment',
  ...
  'real_world_intervention_provenance',
  ...
  'eligibility_source_population_exclusion_awareness_invitation_and_reachability_assurance',
  'latent_need_never_attempted_request_intake_identity_documentation_and_logging_governance',
];

const EXPECTED_PC33_METRICS = {
  world_count: 8,
...
const FALSE_PC33_CLASSIFICATIONS = [
...
function summarizePc33(control, build) {
  return {
    ...
    proof_summary: {
      ...Object.fromEntries(
        Object.keys(EXPECTED_PC33_METRICS).map(key => [key, build.metrics?.[key]])
      ),
      ...Object.fromEntries(
        FALSE_PC33_CLASSIFICATIONS.map(key => [key, build.classification?.[key]])
      ),
...
function buildChain(manifest, baseBuild, extensionControl, openFrontiers, promotionRequirements) {
  ...
  push({
    ...
    event_type: 'pc33_record_identity_entity_boundary_frame_membership_and_denominator_control_admitted',
...
export function validatePreferenceCustodyManifestV31(manifest) {
  ...
  if (manifest?.schema_version !== PREFERENCE_CUSTODY_MANIFEST_V31_SCHEMA_VERSION) {
    errors.push('preference custody v31 manifest schema mismatch');
  }
  if (manifest?.manifest_id !== 'preference-custody-laboratory-floor-v31') {
    errors.push('manifest_id must remain preference-custody-laboratory-floor-v31');
  }
  if (manifest?.control_issue !== 780) errors.push('v31 control issue must remain 780');
  if (manifest?.status !== 'synthetic_control_floor_extension') {
    errors.push('v31 manifest status mismatch');
  }
  ...
  if (base.manifest_id !== 'preference-custody-laboratory-floor-v30') {
    errors.push('v31 base manifest must remain floor v30');
  }
  if (!text(base.source_manifest_path) ||
      base.expected_build_schema !== 'preference-custody-control-manifest-v30-build@1' ||
      base.expected_control_count !== 32) {
    errors.push('v31 base floor contract is incomplete');
  }
  ...
  if (control.fixture_id !== 'same-identity-verified-status-different-provenance-v1') {
    errors.push('PC-33 fixture identity mismatch');
  }
  if (control.failure_class !==
    ...
    errors.push('PC-33 failure class mismatch');
  }
  ...
  if (!text(contract.contract_id) || !text(contract.what_this_is) ||
    ...
    errors.push('v31 interpretation contract is incomplete');
  }
  return errors;
}

export function compilePreferenceCustodyManifestV31(manifest, baseBuild, identityBuild) {
  const errors = validatePreferenceCustodyManifestV31(manifest);
  if (errors.length) {
    throw new Error(`invalid preference custody v31 manifest:\n- ${errors.join('\n- ')}`);
  }
  const baseErrors = validatePreferenceCustodyManifestV30Build(baseBuild);
  if (baseErrors.length) {
    throw new Error(`invalid v30 base build:\n- ${baseErrors.join('\n- ')}`);
  }
  const identityErrors = validatePreferenceIdentityBoundaryAssuranceBuild(identityBuild);
  if (identityErrors.length) {
    throw new Error(`invalid PC-33 build:\n- ${identityErrors.join('\n- ')}`);
  }
  if (baseBuild.manifest_id !== manifest.base_floor.manifest_id) {
    throw new Error('v31 base manifest identity mismatch');
  }
  if (baseBuild.schema_version !== manifest.base_floor.expected_build_schema) {
    throw new Error('v31 base build schema mismatch');
  }
  if (baseBuild.control_count !== manifest.base_floor.expected_control_count) {
    throw new Error('v31 base control count mismatch');
  }
  if (identityBuild.fixture_id !== manifest.extension_control.fixture_id) {
    throw new Error('v31 PC-33 fixture identity mismatch');
  }
  if (identityBuild.schema_version !== manifest.extension_control.expected_build_schema) {
    throw new Error('v31 PC-33 build schema mismatch');
  }

  const extensionControl = summarizePc33(manifest.extension_control, identityBuild);
  ...
  return {
    ...
    controls,
    control_integrity: {
      base_floor_qualified: baseBuild.status === 'laboratory_floor_v30_qualified',
...
export function validatePreferenceCustodyManifestV31Build(compiled) {
  const errors = [];
  if (compiled?.schema_version !== PREFERENCE_CUSTODY_MANIFEST_V31_BUILD_SCHEMA_VERSION) {
    errors.push('preference custody v31 build schema mismatch');
  }
  if (compiled?.manifest_id !== 'preference-custody-laboratory-floor-v31') {
    errors.push('compiled v31 manifest identity mismatch');
  }
  if (compiled?.control_issue !== 780) {
    errors.push('compiled v31 control issue must remain 780');
  }
  if (compiled?.status !== 'laboratory_floor_v31_qualified') {
    errors.push('compiled v31 status mismatch');
  }
  if (compiled?.graph_effect !== 'none') {
    errors.push('compiled v31 graph_effect must remain none');
  }
  ...
  if (compiled?.real_world_evidence_state !== 'none') {
    errors.push('compiled v31 real_world_evidence_state must remain none');
  }
  if (compiled?.control_count !== 33) {
    errors.push('compiled v31 must preserve thirty-three controls');
  }
  if (!sameMembers(array(compiled?.controls).map(control => control.control_id), REQUIRED_CONTROL_IDS)) {
    errors.push('compiled v31 control IDs are incomplete');
  }

  const composition = object(compiled?.composition);
  if (composition.base_manifest_id !== 'preference-custody-laboratory-floor-v30') {
    errors.push('compiled v31 base manifest mismatch');
  }
  if (composition.base_schema_version !== 'preference-custody-control-manifest-v30-build@1') {
    errors.push('compiled v31 base schema mismatch');
  }
  if (composition.base_control_count !== 32) {
    errors.push('compiled v31 base control count must remain thirty-two');
  }
  if (composition.extension_control_id !== 'PC-33') {
    errors.push('compiled v31 extension control must remain PC-33');
  }
  for (const key of ['base_floor_snapshot_sha256', 'extension_snapshot_sha256']) {
    if (!/^[0-9a-f]{64}$/.test(text(composition[key]))) {
      errors.push(`compiled v31 ${key} is invalid`);
    }
  }
  if (!Number.isInteger(composition.added_promotion_requirement_count) ||
      composition.added_promotion_requirement_count < 48) {
    errors.push('compiled v31 must add at least forty-eight promotion requirements');
  }
  if (composition.final_promotion_requirement_count !==
    ...
    errors.push('compiled v31 promotion requirement counts do not reconcile');
  }
  if (composition.final_promotion_requirement_count !==
      unique(compiled?.promotion_boundary?.real_case_requires).length) {
    errors.push('compiled v31 final promotion count does not match the promotion boundary');
  }
  ...
  for (const key of [
    ...
    'no_preference_change_claim',
    'no_intent_inference',
    'all_required_pc33_refusal_rules_present',
    'complete_identity_boundary_assurance_path_preserved'
  ]) {
    if (integrity[key] !== true) {
      errors.push(`compiled v31 control_integrity.${key} must be true`);
    }
  }
  ...
  if (!pc33) {
    ...
    for (const [key, value] of Object.entries(EXPECTED_PC33_METRICS)) {
      if (proof[key] !== value) errors.push(`PC-33 ${key} must equal ${value}`);
    }
    for (const key of FALSE_PC33_CLASSIFICATIONS) {
      if (proof[key] !== false) errors.push(`PC-33 ${key} must remain false`);
    }
    if (proof.complete_identity_boundary_assurance_supported_in_at_least_one_world !== true) {
      errors.push('PC-33 must preserve one complete identity-boundary assurance path');
    }
  }
  ...
  if (!array(compiled?.identification_requirements).some(item =>
    ...
    errors.push('compiled v31 identity-boundary identification stage is missing');
  }
  if (array(compiled?.open_frontiers).includes(
      'identity_resolution_entity_boundary_and_network_frame_assurance'
  )) {
    errors.push('compiled v31 must remove the resolved identity-boundary frontier');
  }
  for (const frontier of REQUIRED_SUCCESSOR_FRONTIERS) {
    if (!array(compiled?.open_frontiers).includes(frontier)) {
      errors.push(`compiled v31 successor frontier missing: ${frontier}`);
    }
  }
  for (const frontier of REQUIRED_PRESERVED_FRONTIERS) {
    if (!array(compiled?.open_frontiers).includes(frontier)) {
      errors.push(`compiled v31 preserved base frontier missing: ${frontier}`);
    }
  }
  requireFalse(
    compiled?.promotion_boundary?.laboratory_controls_are_real_world_evidence,
    'compiled v31 laboratory_controls_are_real_world_evidence',
    errors
  );
  ...
  for (const event of chain) {
    ...
    if (seen.has(event?.event_id)) {
      errors.push(`duplicate compiled v31 event ${event.event_id}`);
    }
    if (event?.previous_event_sha256 !== previous) {
      errors.push(`compiled v31 event ${event?.event_id} previous hash mismatch`);
    }
    for (const sourceId of array(event?.source_event_ids)) {
      if (!seen.has(sourceId)) {
        errors.push(`compiled v31 event ${event?.event_id} references unseen source ${sourceId}`);
      }
    }
    const unsigned = { ...event };
    delete unsigned.event_sha256;
    if (event?.event_sha256 !== sha256(unsigned)) {
      errors.push(`compiled v31 event ${event?.event_id} hash mismatch`);
    }
    seen.add(event.event_id);
    previous = event.event_sha256;
  }
  if (!/^[0-9a-f]{64}$/.test(text(compiled?.custody_chain_head_sha256))) {
    errors.push('compiled v31 custody head is invalid');
  }
  if (chain.at(-1)?.event_sha256 !== compiled?.custody_chain_head_sha256) {
    errors.push('compiled v31 custody head mismatch');
  }
  if (!text(compiled?.interpretation_contract?.copy_ready_caveat)) {
    errors.push('compiled v31 caveat is required');
  }
  return errors;
}
...
export function renderPreferenceCustodyManifestV31Markdown(compiled) {
  ...
  const lines = [
    ...
    '',
    '## PC-33: identity resolution, entity-boundary, and network frame',
    ''
  ];
