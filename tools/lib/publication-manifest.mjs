...
function extractReferences(relative, source) {
  ...
  const values = [];
  if (extension === '.html') {
    // Preserve script-tag attributes (especially src) while refusing to parse
    // inline JavaScript bodies as HTML dependency declarations.
    const htmlSource = source.replace(/(<script\b[^>]*>)[\s\S]*?<\/script>/gi, '$1</script>');
...
function resolveReference(root, fromRelative, raw) {
  ...
  const candidate = reference.startsWith('/')
    ? reference.slice(1)
    : path.posix.join(path.posix.dirname(fromRelative), reference);
  // Repository-root navigation such as ../#case/... resolves to the root
  // index rather than becoming an invalid empty/dot repository path.
  const joined = candidate === '' || candidate === '.' || candidate === './'
    ? 'index.html'
    : normalizeRepositoryPath(candidate, `${fromRelative} reference`);
...
export function validatePublicationPlan(plan, { root = moduleRoot } = {}) {
  ...
  for (const [index, rule] of (plan?.normalizations || []).entries()) {
    if (!rule?.normalization_id || normalizationIds.has(rule.normalization_id)) fail(`normalizations[${index}]: unique normalization_id required`);
    normalizationIds.add(rule?.normalization_id);
    if (!Array.isArray(rule?.paths) || !rule.paths.length) fail(`normalizations[${index}]: paths required`);
    if (rule?.field !== 'generated') fail(`normalizations[${index}]: only generated metadata may be normalized`);
    if (typeof rule?.value !== 'string' || !rule.value) fail(`normalizations[${index}]: stable string value required`);
    if (rule?.publication_status !== 'public_normalized_projection') fail(`normalizations[${index}]: publication status drift`);
    if (rule?.authority_ceiling !== 'publication_projection_only_not_evidence_authority') fail(`normalizations[${index}]: authority ceiling drift`);
    for (const raw of rule?.paths || []) {
      try {
        const relative = normalizeRepositoryPath(raw, `normalizations[${index}].paths`);
        if (normalizedPaths.has(relative)) fail(`${relative}: duplicate publication normalization`);
        normalizedPaths.add(relative);
        if (!entryPaths.has(relative)) fail(`${relative}: normalized path must be an exact public entry`);
        if (path.posix.extname(relative) !== '.json') fail(`${relative}: normalized public projection must be JSON`);
      } catch (error) { fail(error.message); }
    }
  }
...
function normalizationMap(plan) {
  const map = new Map();
  for (const rule of plan.normalizations || []) {
    for (const relative of rule.paths || []) map.set(normalizeRepositoryPath(relative, `normalization ${rule.normalization_id}`), rule);
  }
  return map;
}

function copyFile(root, destination, relative, plan) {
  const source = path.join(root, relative);
  const target = path.join(destination, relative);
  fs.mkdirSync(path.dirname(target), { recursive: true });
  const rule = normalizationMap(plan).get(relative);
  if (!rule) {
    fs.copyFileSync(source, target);
    return;
  }
  const parsed = JSON.parse(fs.readFileSync(source, 'utf8'));
  if (!Object.hasOwn(parsed, rule.field)) throw new Error(`${relative}: declared normalization field ${rule.field} is absent`);
  parsed[rule.field] = rule.value;
  fs.writeFileSync(target, stable(parsed));
}

function computeManifest({ root, destination, plan, sourcePaths }) {
  const sourceSet = new Set(sourcePaths);
  const generatedSet = new Set(plan.generated_outputs || []);
  const normalizedSet = new Set(normalizationMap(plan).keys());
  ...
  const entries = files.map(relative => {
    ...
    return {
      ...
      publication_status: sourceSet.has(relative) ? (normalizedSet.has(relative) ? 'public_normalized_projection' : 'public') : 'generated_public_output',
    };
  });
  ...
  return {
    ...
    counts: {
      ...
      normalized_source_files: entries.filter(entry => entry.publication_status === 'public_normalized_projection').length,
    ...
    boundaries: {
      ...
      normalization_changes_evidence_authority: false,
      graph_effect: 'none',
    },
  };
}
...
export function buildPublicationArtifact({ root = moduleRoot, destination = path.join(root, 'dist') } = {}) {
  ...
  for (const relative of sourcePaths) copyFile(root, destination, relative, plan);
...
export function validatePublicationArtifact({ root = moduleRoot, destination = path.join(root, 'dist') } = {}) {
  ...
  const publicJavascript = listFiles(destination).filter(relative => /\.(?:js|mjs)$/.test(relative));
  for (const relative of publicJavascript) {
    const source = fs.readFileSync(path.join(destination, relative), 'utf8');
    if (/(?:fetch|loadJson|readData)\(\s*['"](?:\.\/)?graph\.json['"]/.test(source)) fail(`${relative}: public runtime requests the retired generic graph`);
  }
