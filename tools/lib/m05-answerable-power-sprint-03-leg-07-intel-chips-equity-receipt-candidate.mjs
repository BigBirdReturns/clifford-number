...
export const INTEL_CHIPS_INSTRUMENT_QUANTITIES=Object.freeze({
  accelerated_disbursement_authority_usd:8869800000,
  closing_common_shares_issued:274583000,
  initial_escrowed_shares:158740000,
  conditional_warrant_max_shares:240516150,
  resale_registered_common_shares:673839150,
  resale_registered_warrant_max_shares:240516150,
  q3_2025_escrow_release_shares:684000,
  fiscal_2025_escrow_releases_approximate:3000000,
  q2_2026_quarter_escrow_releases_approximate:7000000,
  first_half_2026_escrow_releases_approximate:13000000,
  cumulative_escrow_releases_through_2026_06_27_approximate:16000000,
  escrow_shares_remaining_2026_06_27_approximate:143000000
});

export const INTEL_CHIPS_RESALE_REGISTRATION_LOCATOR=[
  'Registration of potential resale by the United States Department of Commerce of a warrant to purchase up to 240,516,150 shares and 673,839,150 shares of common stock',
  'The 673,839,150 registered common shares comprise 274,583,000 delivered shares, 158,740,000 escrowed shares, and 240,516,150 shares issuable upon warrant exercise',
...
export function summarizeIntelChipsEquityReceiptCandidate(
  ...
  return {
    ...
    intel_cumulative_escrow_releases_approximate:
      candidate?.receipt?.instrument_quantities
        ?.cumulative_escrow_releases_through_2026_06_27_approximate??null,
    intel_resale_registered_common_shares:
      candidate?.receipt?.instrument_quantities?.resale_registered_common_shares??null,
...
export function validateIntelChipsEquityReceiptCandidate(
  ...
  const resaleSource=sources.find(
    (row)=>row.source_id==='US-INTEL-CHIPS-RESALE-REGISTRATION'
  );
  check(
    same(resaleSource?.locator,INTEL_CHIPS_RESALE_REGISTRATION_LOCATOR),
    'Intel resale-registration quantity locator drift'
  );

  const quantities=receipt.instrument_quantities||{};
  check(
    same(quantities,INTEL_CHIPS_INSTRUMENT_QUANTITIES),
    'Intel instrument quantity ledger drift'
  );
  check(
    quantities.resale_registered_common_shares===
      quantities.closing_common_shares_issued+
      quantities.initial_escrowed_shares+
      quantities.conditional_warrant_max_shares,
    'Intel resale-registration component arithmetic drift'
  );
  check(
    quantities.cumulative_escrow_releases_through_2026_06_27_approximate===
      quantities.fiscal_2025_escrow_releases_approximate+
      quantities.first_half_2026_escrow_releases_approximate,
  ...
  check(existing.robodebt_pre_action_timing===true,'existing Robodebt pre-action advancement drift');
  check(existing.robodebt_durability===false,'existing Robodebt durability drift');
  ...
  for(const key of [
    ...
    'period_release_is_cumulative_release',
