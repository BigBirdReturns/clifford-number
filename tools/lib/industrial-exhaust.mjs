...
const LEGACY_PHONE_SPAN_PATTERN = /(?:[+＋](?=\s*[0-9０-９])|[（(](?=\s*[0-9０-９]))?\s*[0-9０-９][0-9０-９()./／\s\-‐‑‒–—−－．（）]{5,}[0-9０-９](?:\s*[)）])*(?=$|[^\p{L}\p{N}]|[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]|(?:(?:ext(?:ension)?\.?|x)\s*[.:#：＃]?\s*[0-9０-９]))/giu;
...
const NON_LEGACY_PHONE_WRAPPER_PATTERN = /[\[\]{}［］｛｝【】]/u;
...
function leadingUrlOriginStart(value) {
  let start = 0;
  for (const character of value) {
    // A plus sign may be the true left edge of an international telephone, and
    // a slash may be the true left edge of a scheme-relative URL. Neither can
    // be consumed as generic wrapper punctuation.
    if (character === '+'
        || character === '/'
        || /[\p{L}\p{N}]/u.test(character)) break;
    if (!URL_TOKEN_LEADING_WRAPPER_PATTERN.test(character)) break;
...
function directUrlTokenOrigin(value) {
  ...
  if (unsupported && !/^https?$/iu.test(unsupported[1])) {
    return {
      normalized,
      originStart: unsupported.index + unsupported[0].length
        - unsupported[1].length - 3,
      approved: false,
      unsupported: true
    };
  }
...
function hasUnsupportedUrlSchemeOrigin(value) {
  return directUrlTokenOrigin(value).unsupported;
}

function hasApprovedDirectUrlToken(value) {
  return directUrlTokenOrigin(value).approved;
}
...
function hasUrlTokenPrefixContext(normalizedPrefix) {
  ...
  // refusal controls rely on this conservative interpretation. A complete
  // token origin, when available, outranks any interior suffix interpretation.
  const tokenStart = nonWhitespaceTokenStart(normalizedPrefix);
  const terminalToken = normalizedPrefix.slice(tokenStart);
  const origin = directUrlTokenOrigin(terminalToken);
  if (origin.unsupported) return false;
  if (origin.approved) return true;
  // Absolute and scheme-relative origins are decided at the indexed token
  // boundary. This legacy suffix fallback remains host-shaped so interior `//`
  // punctuation in a neutral token cannot manufacture URL custody.
  return /(?:www\.|(?:[\p{L}\p{N}](?:[\p{L}\p{N}-]*[\p{L}\p{N}])?\.)+(?:[\p{L}]{2,}|xn--[\p{L}\p{N}-]{2,})(?=\.?[:/?#])|(?:\d{1,3}\.){3}\d{1,3}(?=\.?[:/?#]))[^\s]*$/iu.test(
    terminalToken
  );
}
...
function hasUrlTokenBoundaryPrefixContext(normalizedPrefix) {
  ...
  // begin at the terminal token boundary, after at most a token-leading wrapper
  // run. This prevents a dotted identifier suffix from minting IPv4 custody.
  const tokenStart = nonWhitespaceTokenStart(normalizedPrefix);
  const terminalToken = normalizedPrefix.slice(tokenStart);
  return directUrlTokenOrigin(terminalToken).approved;
}
...
function nonWhitespaceTokenStart(
  value,
  end = value.length,
  tokenBoundaryIndex = null
) {
  const boundedEnd = Math.min(Math.max(0, end), value.length);
  if (tokenBoundaryIndex) return tokenBoundaryIndex.start(boundedEnd);
  let start = boundedEnd;
  while (start > 0 && !/\s/u.test(value[start - 1])) start -= 1;
  return start;
}

function previousNonWhitespaceTokenBounds(
  value,
  end = value.length,
  tokenBoundaryIndex = null
) {
  let tokenEnd = tokenBoundaryIndex
    ? tokenBoundaryIndex.previousEnd(end)
    : Math.min(Math.max(0, end), value.length);
  if (!tokenBoundaryIndex) {
    while (tokenEnd > 0 && /\s/u.test(value[tokenEnd - 1])) tokenEnd -= 1;
  }
  if (tokenEnd <= 0) return null;
  return {
    start: nonWhitespaceTokenStart(
      value,
      tokenEnd,
      tokenBoundaryIndex
    ),
...
function createNonWhitespaceTokenBoundaryIndex(value) {
  ...
  const urlOriginStarts = new Int32Array(value.length + 1);
  urlOriginStarts.fill(-1);
  let tokenStart = 0;
  let previousEnd = 0;

  for (let index = 0; index <= value.length; index += 1) {
    const atEnd = index === value.length;
    const whitespace = !atEnd && /\s/u.test(value[index]);
    if ((atEnd || whitespace) && tokenStart < index) {
      ...
      urlTokenStates[tokenStart] = origin.approved
        ? 1
        : origin.unsupported
          ? -1
          : 0;
      if (origin.approved) {
        urlOriginStarts[tokenStart] = tokenStart + sourceEndForNormalizedPrefix(
          value.slice(tokenStart, index),
          origin.originStart
        );
      }
    }
    ...
    const end = index + 1;
    if (whitespace) tokenStart = end;
    else previousEnd = end;
    tokenStarts[end] = tokenStart;
    previousEnds[end] = previousEnd;
  }

  return {
    start(end) {
      const boundedEnd = Math.min(Math.max(0, end), value.length);
      return tokenStarts[boundedEnd];
    },
    previousEnd(end) {
      const boundedEnd = Math.min(Math.max(0, end), value.length);
    ...
    urlState(start) {
      const boundedStart = Math.min(Math.max(0, start), value.length);
      return urlTokenStates[boundedStart];
    },
    urlOriginStart(start) {
      const boundedStart = Math.min(Math.max(0, start), value.length);
      return urlOriginStarts[boundedStart];
    },
    urlStateAt(start, offset) {
      const state = this.urlState(start);
      if (state !== 1) return state;
      const originStart = this.urlOriginStart(start);
      const boundedOffset = Math.min(Math.max(0, offset), value.length);
      return originStart >= 0 && boundedOffset >= originStart ? 1 : 0;
    },
    isApprovedUrlAt(start, offset) {
      return this.urlStateAt(start, offset) === 1;
    },
    isUnsupportedUrlTokenStart(start) {
      return this.urlState(start) === -1;
    }
  };
}
...
function redactionPrefixContext(
  ...
  const currentToken = previousNonWhitespaceTokenBounds(
    input,
    boundedOffset,
    tokenBoundaryIndex
  );
  const tokenStart = currentToken?.start ?? boundedOffset;
  const contactTokenStart = tokenBoundaryIndex && boundedOffset < input.length
    ? tokenBoundaryIndex.start(boundedOffset + 1)
    : tokenStart;
  const contactUrlTokenState = tokenBoundaryIndex
    ? tokenBoundaryIndex.urlStateAt(contactTokenStart, boundedOffset)
    : 0;
  ...
  if (contactUrlTokenState === 1
    ...
    return {
      ...
      identifierProtectedContext: false,
      urlTokenState: 1
    };
  }

  let contextStart = tokenBoundaryIndex ? boundedStart : Math.min(
    boundedTokenStart,
    tokenStart
  );
  if (tokenBoundaryIndex?.isApprovedUrlAt(
    boundedTokenStart,
    boundedOffset
  )) {
    contextStart = Math.min(contextStart, boundedTokenStart);
  }
  if (currentToken
      && tokenBoundaryIndex?.isApprovedUrlAt(
        currentToken.start,
        currentToken.end
      )) {
    contextStart = Math.min(contextStart, currentToken.start);
  }

  // An accepted wrapper sequence can be arbitrarily deeper than the ordinary
  // context window. Walk only that terminal sequence, then inspect one bounded
  // label window at its source origin. This preserves explicit identifier and
  // telephone labels without restoring full-token work for ordinary long
  // non-whitespace candidates.
  if (tokenBoundaryIndex) {
    let wrapperCursor = boundedOffset;
    let sawOpeningWrapper = false;
    while (wrapperCursor > 0) {
      const character = previousCodePoint(input, wrapperCursor);
      if (/\s/u.test(character)) {
        wrapperCursor -= character.length;
        continue;
      }
    ...
    if (sawOpeningWrapper) {
      const wrapperTokenStart = tokenBoundaryIndex.start(wrapperCursor);
      const probeStart = Math.max(wrapperTokenStart, wrapperCursor - 256);
      const labelProbe = input
        .slice(probeStart, wrapperCursor)
        .normalize('NFKC');
      if (EXPLICIT_IDENTIFIER_LABEL_PATTERN.test(labelProbe)
          || PHONE_LABEL_PATTERN.test(labelProbe)) {
        contextStart = Math.min(contextStart, wrapperTokenStart);
      }
    }
  }
  let scanStart = tokenStart;
  let normalizedToken = !tokenBoundaryIndex && currentToken
    ? input.slice(currentToken.start, currentToken.end).normalize('NFKC')
    : '';

  while (/^(?:\(|\[|\{|（|［|【)+$/u.test(normalizedToken) && scanStart > 0) {
    const priorToken = previousNonWhitespaceTokenBounds(
      input,
      scanStart,
      tokenBoundaryIndex
    );
  ...
  return {
    ...
    urlTokenState: contactUrlTokenState
  };
}
...
function consumeCleanOwnedObservationClosers(separator, openers) {
  const normalized = separator.normalize('NFKC');
  const remaining = [...openers];
  const containsCloser = [...normalized].some(character =>
    OBSERVATION_WRAPPER_CLOSERS.has(character)
  );
  let cursor = 0;
  let sawClosing = false;
  ...
  while (remaining.length) {
    ...
    if (expectedOpener === undefined
        || remaining.at(-1) !== expectedOpener) {
      return {
        valid: false,
        sawClosing: sawClosing || containsCloser,
        openers
      };
    }
    sawClosing = true;
  ...
  return { valid: true, sawClosing, openers: remaining, remainder };
}
...
function identifierOwnedObservationPhoneTransition(
  ...
  if (observationGroup > 0) {
    ...
    if (candidateObservation) {
      ...
      observationIsWeak = isWeakBareRangeObservation(
        observationSource,
        externalSuffix
      );
    }
  ...
  // A weak bare range owns its exact source bytes but supplies no telephone
  // authority. Permit progression only when a source-distinct slash or a
  // wrapper closer already owned by that observation separates its proved
  // endpoint from a later telephone that independently validates. Unowned or
  // mismatched closers remain inert, preserving the same-glyph dash refusal.
  if (observationIsWeak) {
    const progressionBoundarySource = candidate.slice(
      observation.end,
      laterPhone.start
    );
    const progressionBoundary = progressionBoundarySource.normalize('NFKC');
    const observationOpeners = trailingObservationOpeners(
      candidate.slice(
        initialIdentifier.end,
        groups[observation.group].index
      )
    );
    ...
    const sourceDistinctBoundary = closingState.valid
      && (/[\/／]/u.test(closingState.remainder ?? progressionBoundary)
        || closingState.sawClosing);
    if (!sourceDistinctBoundary
        || /[\p{L}\p{N}]/u.test(progressionBoundary)) {
      return {
        initialIdentifier,
        observation,
        laterPhone: null,
        refuseCandidate: true
      };
    }
  }

  return { initialIdentifier, observation, laterPhone };
}
...
function phoneRedactionRanges(
  ...
  candidateEntryUrlContextOverride = null
) {
  const ranges = [];
  ...
  const candidateEntryUrlContext = candidateEntryUrlContextOverride === null
    ? hasUrlTokenPrefixContext(
        entryCombinedCandidate.slice(entryTrailingTokenStart).normalize('NFKC')
      )
    : Boolean(candidateEntryUrlContextOverride);
...
const identifierOwnedTransitionBeforePartition = unoverriddenIdentifierContext
  ...
  : null;
if (identifierOwnedTransitionBeforePartition?.refuseCandidate) return [];
...
function redactPhoneSubspans(
  ...
  return renderPhoneRedactionRanges(
    ...
    phoneRedactionRanges(
      ...
      indeterminatePhoneContext,
      inheritedExplicitPhoneLabelContext,
      candidateEntryUrlContextOverride
    )
  );
}
...
function createParenthesisDepthIndex(value) {
  ...
  const narrativeBoundaryPotential = new Uint8Array(value.length + 1);
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    const delta = normalized === '('
      ...
      : normalized === ')'
        ? -1
        : 0;
    const potentialNarrativeBoundary = normalized === '.'
      || normalized === '!'
      || normalized === '?'
      || normalized === '。'
      || normalized === '！'
    ...
    narrativeBoundaryPotential[index + 1] = narrativeBoundaryPotential[index]
      || Number(potentialNarrativeBoundary);
  }
  if (!hasParentheses) {
    return {
  ...
  return {
    hasParentheses: true,
    hasPotentialNarrativeBoundary(end) {
      const boundedEnd = Math.max(0, Math.min(value.length, end));
      return narrativeBoundaryPotential[boundedEnd] !== 0;
    },
    depth(start, end) {
...
function currentNarrativeParenthesisDepth(value, end, parenthesisDepthIndex) {
  ...
  // The exact predecessor parser can change a period decision as the query
  // endpoint advances, because abbreviation adjudication inspects only the
  // source present in that callback prefix. Preserve that query-local grammar
  // whenever any sentence or paragraph boundary glyph is present. When none
  // exists, the narrative context necessarily begins at source offset zero,
  // so the shared source index supplies the depth without copying or scanning
  // the complete growing prefix for every callback.
  if (!parenthesisDepthIndex.hasPotentialNarrativeBoundary(boundedEnd)) {
    return parenthesisDepthIndex.depth(0, boundedEnd);
  }
...
function isIdentifierWeakRangeAtomicCandidate(
  candidate,
  offset,
  input,
  tokenBoundaryIndex
) {
  if (!NON_LEGACY_PHONE_WRAPPER_PATTERN.test(candidate)) return false;
  ...
  if (prefixContext.indeterminate
      || !hasExplicitIdentifierPrefixNormalized(normalizedPrefix)
      || hasPhoneLabelPrefixNormalized(normalizedPrefix)) {
    return false;
  }
  ...
  const externalSuffix = input.slice(
  ...
  for (let index = 1; index < groups.length; index += 1) {
    ...
    const protectedDigits = candidate.slice(0, previousEnd)
      .normalize('NFKC')
      .replace(/\D/gu, '')
      .length;
    if (protectedDigits < 7) continue;
    ...
    const observation = crossCallbackObservationMatch(
      observationSource,
      externalSuffix
    );
    if (observation
...
function redactPhoneSpanCandidate(
  ...
  tokenBoundaryIndex = createNonWhitespaceTokenBoundaryIndex(input)
) {
  const firstContactCharacter = candidate.search(/[+＋(（0-9０-９]/u);
  const contactOffset = offset + Math.max(0, firstContactCharacter);
  const candidateEnd = offset + candidate.length;
...
export function redactContactData(value) {
  ...
  const tokenBoundaryIndex = createNonWhitespaceTokenBoundaryIndex(
    emailRedacted
  );

  const processPhoneCandidate = (candidate, offset, input) => {
  ...
  const phoneRedacted = emailRedacted.replace(
    ...
    (candidate, offset, input) => {
      if (NON_LEGACY_PHONE_WRAPPER_PATTERN.test(candidate)
          && !isIdentifierWeakRangeAtomicCandidate(
            candidate,
            offset,
            input,
            tokenBoundaryIndex
        ...
        return candidate.replace(
          LEGACY_PHONE_SPAN_PATTERN,
          (legacyCandidate, legacyOffset) => processPhoneCandidate(
            legacyCandidate,
            offset + legacyOffset,
            input
          )
      ...
      return processPhoneCandidate(candidate, offset, input);
    }
  );
