...
export function validateSchoolhouseHonorFoundation990Custody(dir = DEFAULT_DIR) {
  ...
  check(manifest.counts.source_inventory_rows === 480 && manifest.counts.coverage_denominator_rows === 30 && manifest.counts.explicit_gap_rows === 16, 'Form 990 manifest headline denominator drift');
  check(manifest.source_inventory.evidence_class_counts.official === 255 && manifest.source_inventory.evidence_class_counts.primary_public === 54 && manifest.source_inventory.evidence_class_counts.primary_public_state_charity_registry_route_custody === 6, 'Form 990 manifest provenance count drift');
  ...
  check(manifest.storage_contract.source_inventory_parts.at(-3) === 'source-inventory-18.jsonl' && manifest.storage_contract.source_inventory_parts.at(-2) === 'source-inventory-19.jsonl' && manifest.storage_contract.source_inventory_parts.at(-1) === 'source-inventory-20.jsonl', 'Form 990 source inventory tail drift');
