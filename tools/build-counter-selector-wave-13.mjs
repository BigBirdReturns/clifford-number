...
export const releaseScope = [
  ".github/workflows/counter-selector-wave-13.yml",
  "data/project/counter-selector-wave-13-blind-review.json",
  "schemas/counter-selector-blind-review-b04.schema.json",
  "docs/methods/counter-selector-blind-review-b04.md",
  "docs/milestones/counter-selector-wave-13.md",
  "tools/build-counter-selector-wave-13.mjs",
  "tools/validate-counter-selector-wave-13.mjs",
  "test/counter-selector-wave-13.test.js"
];
...
export function computeReleaseManifest() {
  ...
  return {
    schema_version: 'counter-selector-wave-13-release-manifest@1',
...
export function deriveRegistry(contract) {
  return {
    schema_version: 'counter-selector-blind-review-b04-registry@1',
    program_id: contract.program_id,
    wave_id: contract.wave_id,
    batch_id: contract.batch_id,
    as_of: contract.as_of,
    status: '5_internal_blind_reviews_complete_no_complete_operator_finding',
...
export function deriveDisagreementLedger(contract) {
  return {
    schema_version: 'counter-selector-review-disagreement-b04-ledger@1',
    program_id: contract.program_id,
    wave_id: contract.wave_id,
    batch_id: contract.batch_id,
    as_of: contract.as_of,
    status: '5_disagreements_preserved_without_averaging',
    counts: {
...
export function deriveReport(contract, registry, ledger, manifest) {
  return {
    schema_version: 'counter-selector-wave-13-report@1',
    program_id: contract.program_id,
    wave_id: contract.wave_id,
    batch_id: contract.batch_id,
    as_of: contract.as_of,
    ...
    release_manifest: {
      path: 'data/project/counter-selector-wave-13-release-manifest.json',
...
export function renderHtml(report) {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Counter-Selector Wave 13</title>
...
<h1>Refusal is evidence, not completion</h1>
...
export function buildCounterSelectorWave13() {
  ...
  writeJson('data/project/counter-selector-blind-review-b04-registry.json', registry);
  writeJson('data/project/counter-selector-review-disagreement-b04-ledger.json', ledger);
  const manifest = computeReleaseManifest();
  writeJson('data/project/counter-selector-wave-13-release-manifest.json', manifest);
  const report = deriveReport(contract, registry, ledger, manifest);
  writeJson('reports/core-thesis/counter-selector-wave-13/data.json', report);
  writeText('reports/core-thesis/counter-selector-wave-13/index.html', renderHtml(report));
  console.log(`build-counter-selector-wave-13: ${registry.packet_results.length} packets, ${registry.counts.bounded_dimension_supports} bounded supports, ${registry.counts.operator_findings} operator findings`);
}

if (process.argv[1] === fileURLToPath(import.meta.url)) buildCounterSelectorWave13();
