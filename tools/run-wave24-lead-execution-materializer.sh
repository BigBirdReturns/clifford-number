...
branch='agent/lake-allocator-war-lead-execution-wave-24'
base='748dfe258cd39195375135d748e6af3e7f11a242'
trigger='.github/tmp/wave24-lead-execution-trigger.json'
temporary_workflow='.github/workflows/temporary-wave24-lead-execution-materializer.yml'
runner='tools/run-wave24-lead-execution-materializer.sh'
...
const wave24PolicyPath = 'data/project/lake-allocator-war-lead-execution-wave-24-policy.json';
const wave24 = readJson(wave24PolicyPath);
const wave23Projection = readJson(wave24.paths.source_projection);
const ledgerPaths = wave23Projection.packets
  .slice()
  .sort((a, b) => a.packet_sequence - b.packet_sequence)
  .map(packet => wave24.paths.ledger_root + '/' + packet.source_queue_ref.toLowerCase() + '.jsonl');
...
const required = {
  'allocator-war-source': [
    wave24PolicyPath,
    wave24.paths.source_plan,
    wave24.paths.method,
    wave24.paths.milestone,
    ...ledgerPaths
  ],
  'allocator-war-lake-actions': [wave24.paths.projection],
  'allocator-war-reports': [wave24.paths.report]
};
...
for (const relative of [wave24.paths.projection, ...ledgerPaths]) {
  addSorted(policy.projection_contract.allowed_generated_paths, relative);
}
policy.boundaries.wave_24_acquisition_record_is_evidence_row = false;
policy.boundaries.wave_24_acquisition_disposition_is_evidence_result = false;
policy.boundaries.wave_24_gate_unspecified_is_institutional_gate = false;
policy.boundaries.wave_24_partial_recovery_is_complete_denominator = false;
...
lakePolicy.boundaries.wave_24_acquisition_record_proves_evidence = false;
lakePolicy.boundaries.wave_24_gate_unspecified_proves_institutional_gate = false;
writeJson(lakePolicyPath, lakePolicy);

const packagePath = 'package.json';
const pkg = readJson(packagePath);
pkg.scripts['build:lake-allocator-war-lead-execution-wave-24'] = 'node tools/build-lake-allocator-war-lead-execution-wave-24.mjs';
pkg.scripts['validate:lake-allocator-war-lead-execution-wave-24'] = 'node tools/validate-lake-allocator-war-lead-execution-wave-24.mjs && node test/lake-allocator-war-lead-execution-wave-24.test.js';
pkg.scripts['ci:lake-allocator-war-lead-execution-wave-24'] = 'npm run build:lake-allocator-war-lead-execution-wave-24 && npm run validate:lake-allocator-war-lead-execution-wave-24';
if (!pkg.scripts.check.includes('validate:lake-allocator-war-lead-execution-wave-24')) {
  const marker = 'npm run validate:lake-allocator-war-lead-acquisition-wave-23';
  if (!pkg.scripts.check.includes(marker)) throw new Error('Wave 23 release-gate marker missing');
  pkg.scripts.check = pkg.scripts.check.replace(marker, marker + ' && npm run validate:lake-allocator-war-lead-execution-wave-24');
}
writeJson(packagePath, pkg);

const installerPath = 'tools/install-lake-allocator-war-wave-21.mjs';
let installer = fs.readFileSync(installerPath, 'utf8');
if (!installer.includes(wave24PolicyPath)) {
  const marker = "  'docs/milestones/lake-allocator-war-lead-acquisition-wave-23.md'\n]) roots.add(relative);";
  if (!installer.includes(marker)) throw new Error('Wave 21 installer Wave 23 marker missing');
  const insertion = [
    wave24PolicyPath,
    wave24.paths.source_plan,
    wave24.paths.projection,
...
if (!validator21.includes(wave24PolicyPath)) {
  const sourceMarker = "      'docs/milestones/lake-allocator-war-lead-acquisition-wave-23.md'\n    ]],";
  ...
  const actionsMarker = "['allocator-war-lake-actions', [policy.paths.projection, policy.paths.reconciliation, 'build/lake-actions/allocator-war-estate-execution-wave-22.json', 'build/lake-actions/allocator-war-lead-acquisition-wave-23.json']],";
  const actionsReplacement = "['allocator-war-lake-actions', [policy.paths.projection, policy.paths.reconciliation, 'build/lake-actions/allocator-war-estate-execution-wave-22.json', 'build/lake-actions/allocator-war-lead-acquisition-wave-23.json', '" + wave24.paths.projection + "']],";
  ...
  const reportsMarker = "['allocator-war-reports', [policy.paths.report, 'reports/lake-allocator-war-estate-execution-wave-22.md', 'reports/lake-allocator-war-lead-acquisition-wave-23.md']]";
  const reportsReplacement = "['allocator-war-reports', [policy.paths.report, 'reports/lake-allocator-war-estate-execution-wave-22.md', 'reports/lake-allocator-war-lead-acquisition-wave-23.md', '" + wave24.paths.report + "']]";
  ...
  const rootMarker = "    'docs/milestones/lake-allocator-war-lead-acquisition-wave-23.md'\n  ]) if (!lakePolicy.authoritative_roots.includes(relative))";
...
const buildMarker = '3.24 **Allocator-war lead acquisition execution — Wave 24.**';
...
const readmeMarker = '## Allocator-war lead acquisition execution';
...
node --check tools/build-lake-allocator-war-lead-execution-wave-24.mjs
node --check tools/validate-lake-allocator-war-lead-execution-wave-24.mjs
node --check test/lake-allocator-war-lead-execution-wave-24.test.js
...
first_projection="$(sha256sum build/lake-actions/allocator-war-lead-execution-wave-24.json | cut -d' ' -f1)"
first_report="$(sha256sum reports/lake-allocator-war-lead-execution-wave-24.md | cut -d' ' -f1)"
first_ledgers="$(find data/acquisition/lake-allocator-war-wave-24 -type f -name '*.jsonl' -print0 | sort -z | xargs -0 sha256sum | sha256sum | cut -d' ' -f1)"
node tools/build-lake-allocator-war-lead-execution-wave-24.mjs
test "$first_projection" = "$(sha256sum build/lake-actions/allocator-war-lead-execution-wave-24.json | cut -d' ' -f1)"
test "$first_report" = "$(sha256sum reports/lake-allocator-war-lead-execution-wave-24.md | cut -d' ' -f1)"
test "$first_ledgers" = "$(find data/acquisition/lake-allocator-war-wave-24 -type f -name '*.jsonl' -print0 | sort -z | xargs -0 sha256sum | sha256sum | cut -d' ' -f1)"
git add -A
git diff --check
...
fixed_point_pass() {
  ...
  LAW24_SKIP_SHARDS=1 node tools/validate-lake-allocator-war-lead-execution-wave-24.mjs
  git add -A
}

for pass in 1 2 3; do
  fixed_point_pass "Wave 24 fixed-point pass $pass"
done
...
for pass in 1 2 3; do
  fixed_point_pass "Wave 24 post-release seal pass $pass"
done
validate_epoch
...
git commit -m 'Execute allocator-war lead acquisition Wave 24'
