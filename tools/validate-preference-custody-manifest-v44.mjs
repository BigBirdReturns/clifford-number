...
import { validatePreferenceCustodyManifestV44, validatePreferenceCustodyManifestV44Build } from './lib/preference-custody-manifest-v44.mjs';
const manifestPath = process.argv[2] ?? 'data/research/preference-custody/control-manifest-v44.json';
const buildPath = process.argv[3] ?? 'build/research/preference-custody-laboratory-floor-v44.json';
...
const errors=[...validatePreferenceCustodyManifestV44(manifest),...validatePreferenceCustodyManifestV44Build(build,manifest,baseBuild,targetBuild,targetFixture,baseSources)];
if(errors.length){console.error(errors.map(error=>`- ${error}`).join('\n'));process.exit(1);}
console.log('validated Preference Custody laboratory floor v44');
