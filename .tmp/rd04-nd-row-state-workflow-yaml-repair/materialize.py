...
SOURCE_PRODUCT_TREE = "35dfc711caf69844a3de93a99b763d32988e29be"
...
STAGED_WORKFLOW_OBJECT = ".tmp/rd04-nd-row-state-workflow-yaml-repair/standing-workflow.yml"
...
REPAIRED_PATHS = sorted([PRODUCT_WORKFLOW, TEST_PATH, MANIFEST_PATH])
...
def patch_workflow(path: pathlib.Path) -> None:
    ...
    repaired = "          printf '%s\\n' \"$EXPECTED\" > \"$OUT/product-paths.txt\"\n"
    text = replace_once(text, broken, repaired, "workflow printf YAML continuation")
    assert "\n' \"$EXPECTED\" > \"$OUT/product-paths.txt\"" not in text
...
def patch_test(path: pathlib.Path) -> None:
    ...
    text = replace_once(
        ...
        "test child-process import",
    )
    anchor = """test('deterministic builder reproduces all five derived files',()=>{
...
"""
    inserted = anchor + """test('standing qualification workflow parses as YAML',()=>{
  const workflowPath=path.join(ROOT,rel.workflow);
  const parsed=spawnSync('ruby',['-e',"require 'yaml'; YAML.load_file(ARGV.fetch(0)); puts 'yaml_ok'",workflowPath],{encoding:'utf8'});
  assert.equal(parsed.status,0,parsed.stderr||parsed.stdout||'Ruby Psych rejected standing workflow');
  assert.match(parsed.stdout,/yaml_ok/);
});

"""
    text = replace_once(text, anchor, inserted, "test workflow YAML parse")
    path.write_text(text, encoding="utf-8")
...
def parse_workflow(worktree: pathlib.Path) -> str:
    return run(
        [
            "ruby",
            "-e",
            "require 'yaml'; YAML.load_file(ARGV.fetch(0)); puts 'yaml_ok'",
            PRODUCT_WORKFLOW,
        ],
        cwd=worktree,
    )
...
def construct_product() -> dict[str, Any]:
    ...
    patch_workflow(FULL_WORK / PRODUCT_WORKFLOW)
    patch_test(FULL_WORK / TEST_PATH)
    manifest, manifest_data = recompute_manifest(FULL_WORK)

    parse_receipt = parse_workflow(FULL_WORK)
    assert parse_receipt == "yaml_ok"
    for product_path in PRODUCT_PATHS:
    ...
    message = (
        "Repair North Dakota row-state standing workflow YAML\n\n"
        "Keep the printf command inside the YAML run block, add an exact Ruby Psych parse test to the permanent qualification suite, and preserve the recursively schema-qualified one-cell North Dakota row-state transition."
    )
    commit_env = {
    ...
    return {
        ...
        "workflow_bytes": len((FULL_WORK / PRODUCT_WORKFLOW).read_bytes()),
        "workflow_sha256": sha256((FULL_WORK / PRODUCT_WORKFLOW).read_bytes()),
        "workflow_git_blob": git_blob((FULL_WORK / PRODUCT_WORKFLOW).read_bytes()),
...
def qualify(product: dict[str, Any]) -> dict[str, Any]:
    ...
    ruby_version = run(["ruby", "--version"], cwd=QUAL_WORK)
    ...
    (OUT / "ruby-version.txt").write_text(ruby_version + "\n", encoding="utf-8")
    ...
    return {
        "workflow_yaml_parser": "ruby_psych",
...
def stage_objects(product: dict[str, Any]) -> dict[str, Any]:
    ...
    stage_commit = run(
        ...
        cwd=STAGE_WORK,
        env=commit_env,
        input_text="Stage qualified North Dakota row-state workflow YAML repair objects\n\nPublish thirteen non-workflow product paths and one byte-identical, Ruby-Psych-parseable standing-workflow object for repository-authority reconstruction of the qualified fourteen-path tree.",
    )
    ...
    return {
        ...
        "workflow_bytes": product["workflow_bytes"],
        "workflow_sha256": product["workflow_sha256"],
        "workflow_git_blob": product["workflow_git_blob"],
...
def main() -> None:
    ...
    receipt = {
        "schema_version": "ssc-rd04-nd-row-state-workflow-yaml-repair-materialization@1",
        "state": "qualified_workflow_yaml_repair_objects_staged",
        "carrier": carrier,
...
if __name__ == "__main__":
    try:
        ...
        write_json(
            OUT / "failure.json",
            {
                "schema_version": "ssc-rd04-nd-row-state-workflow-yaml-repair-failure@1",
                "state": "failed_closed",
