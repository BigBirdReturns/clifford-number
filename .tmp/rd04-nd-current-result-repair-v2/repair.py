...
def bind_product() -> None:
    ...
    assert exact_status(CANONICAL_PARENT, EXPECTED_PRODUCT_HEAD) == [("A", path) for path in EXPECTED_PATHS]
...
def repair_product() -> dict[str, Any]:
    ...
    run([
        ...
        "origin", f"{new_commit}:refs/heads/{PRODUCT_BRANCH}",
    ], cwd=PRODUCT_WORK)
...
if __name__ == "__main__":
    try:
        ...
        OUT.mkdir(parents=True, exist_ok=True)
        failure = {
            "schema_version": "ssc-rd04-nd-current-result-counter-repair-failure@1",
            "state": "failed_closed",
