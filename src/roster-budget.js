 ...
 * built-ins, no imports — kept dependency-free the same way `src/ui-utils.js`
 ...
 * imported from `src/route-projections.js` / `src/route-diagnostics.js`,
 * matching `src/route-projections.js`'s own precedent of keeping every
 * browser-safe module free of local imports. The only adaptation from that
 * shared logic: a participant's window is `time_start`/`time_end` (no
...
// ---------------------------------------------------------------------------
// Evidence ranking — identical table to src/route-projections.js. Lower
// number = stronger evidence.
const EVIDENCE_RANK = {
  confirmed: 0,
  official: 1,
  government_record: 1,
...
function evidenceRank(evidenceClass) {
  return EVIDENCE_RANK[evidenceClass] ?? 9;
}

function meetsEvidenceFloor(evidenceClass, floor) {
  return evidenceRank(evidenceClass) <= evidenceRank(floor);
}
