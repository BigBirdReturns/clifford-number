...
export function periodBounds(value) {
  ...
  if (validIsoDay(input)) return { start: input, end: input };
  return null;
}
...
export function shortestFilteredPath(hopGraph, start, target, filters = {}) {
  ...
  while (queue.length) {
    ...
    for (const candidate of adjacency.get(current.actor) ?? []) {
      ...
      const orderedBases = [...bases].sort((a, b) => {
        const evidenceDelta = evidenceRank(a.evidence_class) - evidenceRank(b.evidence_class);
        if (evidenceDelta) return evidenceDelta;
        return String(a.surface_label || a.surface_id).localeCompare(String(b.surface_label || b.surface_id));
      });
      const hop = {
        from: current.actor,
        to: candidate.to,
...
export function selectBudgetedParticipants(surface, {
  ...
  const all = actorParticipants(surface);
  const eligible = all.filter(participant => participantMatches(participant, { evidenceFloor, asOf }));
  ...
  return {
    ...
    temporalOrEvidenceFiltered: Math.max(0, all.length - eligible.length),
    totalActors: all.length
  };
}

// Semantic zoom hysteresis prevents representation flicker at threshold boundaries.
const LEVELS = ['corpus', 'machine', 'surface', 'evidence'];
...
export function semanticLevelForScale(scale, previousLevel = null) {
  const value = Math.max(1, Math.min(5.4, Number(scale) || 1));
