const DEFAULT_ENDPOINTS = {
  ...
  boundary: 'build/public-data-boundary.json'
};
...
function formatProjectionBoundary(value) {
...
function renderDataBoundary(boundary) {
  ...
  const bounded = boundary?.views?.bounded_surfaces;
  const hops = boundary?.views?.verified_hops;
  const receipts = boundary?.views?.receipt_graph;
  const set = (selector, value) => { const node = $(selector); if (node) node.textContent = value; };
  set('#boundary-research-value', formatProjectionBoundary(research?.projection_generated));
  set('#boundary-bounded-value', formatProjectionBoundary(bounded?.projection_generated));
  set('#boundary-hop-value', formatProjectionBoundary(hops?.projection_generated));
  set('#boundary-receipt-value', formatProjectionBoundary(receipts?.projection_generated));
  set('#boundary-contract', boundary?.interpretation_contract?.statement
    ?? 'Projection timestamps identify deterministic artifacts; source dates remain in their owning receipts.');
  const root = $('#data-boundary');
  if (root) root.dataset.mixed = String(Boolean(boundary?.alignment?.mixed_projection_boundaries));
}
...
async function init() {
  bindControls();
  try {
    const [researchGraph, hopGraph, surfaceGraph, receiptGraph, dataBoundary] = await Promise.all([
      loadJson(PATHS.research), loadJson(PATHS.hops), loadJson(PATHS.surfaces), loadJson(PATHS.receipts), loadJson(PATHS.boundary)
    ]);
    ...
    renderDataBoundary(dataBoundary);
