...
The master document is a pre-import reference. It says the claims are typed, sourced, and evidence-classed, but that does not make every row public-graph ready. The repo rule is stricter: every edge must remain public-role-only, sourced, validated, and safe to click through.
...
The generated queue is a staging artifact. It does not mutate `graph.json`. It separates ready rows from review rows, verification rows, held rows, and rejected private-field rows.
...
If the answer is no, keep the row in the queue and do not promote it into `graph.json`.
