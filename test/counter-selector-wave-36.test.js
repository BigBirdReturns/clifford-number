...
import { validateSource, EXPECTED_ADJUDICATIONS, EXPECTED_BOUNDARIES, EXPECTED_COUNTS, EXPECTED_JOIN_MATRIX } from '../tools/validate-counter-selector-wave-36.mjs';

const source = JSON.parse(fs.readFileSync('data/project/counter-selector-wave-36-portable-proof-closure.json', 'utf8'));
...
mutations.push(value => { value.as_of = '2026-08-01'; });
mutations.push(value => { value.observed_at = '2026-08-01T00:00:00Z'; });
...
mutations.push(value => { value.join_contract.proofs_may_be_assembled_across_controls = true; });
mutations.push(value => { value.graph_effect = 'changed'; });

assert.equal(mutations.length, 104);
for (const [index, mutate] of mutations.entries()) {
  const candidate = clone(source);
  mutate(candidate);
  assert.throws(() => validateSource(candidate), undefined, `mutation ${index + 1} should fail`);
}
console.log(`counter-selector-wave-36.test: ${mutations.length} adversarial mutations PASS`);
