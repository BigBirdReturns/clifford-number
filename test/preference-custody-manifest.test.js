...
assert.equal(compiled.control_count, 3);
...
assert.ok(compiled.open_frontiers.includes('collective_organization_and_public_standing'));

const markdown = renderPreferenceCustodyManifestMarkdown(compiled);
assert.match(markdown, /Preference custody laboratory floor v1/);
...
assert.ok(validatePreferenceCustodyManifest(missingControl).some(error => /exactly PC-01, PC-02, and PC-03/.test(error)));
