...
const mutations = [
  ...
  ['terminal receipt count', (v) => { v.counts.terminal_class_receipts = 5; }],
  ...
  ['selected class reopened', (v) => { v.selected_classes_open.push({ lane_id: 'RD-02', class_id: 'RD-02-C04', issue: 787, constitutional_exact_label: 'fund-level Green Light, license, leverage commitment, draw, fee, covenant, and amendment chronology', state: 'open', class_closed: false }); }],
  ['all-selected flag cleared', (v) => { v.current_result.all_six_selected_classes_closed = false; }],
  ...
  ['RD-02 state changed', (v) => { v.promoted_class_receipts[5].terminal_state = 'evidence_complete'; }],
  ['RD-02 merge custody changed', (v) => { v.promoted_class_receipts[5].merge_commit = '0'.repeat(40); }],
  ['RD-02 manifest custody changed', (v) => { v.promoted_class_receipts[5].manifest_combined_sha256 = '0'.repeat(64); }],
  ['RD-02 exact-match flag changed', (v) => { v.promoted_class_receipts[5].labels_exact_match = false; }],
  ...
  ['open selected ids added', (v) => { v.current_result.open_selected_class_ids = ['RD-02-C04']; }],
...
const schemaMutations = [
  ...
  ['schema open denominator', (s) => { s.properties.selected_classes_open.minItems = 1; }],
  ['schema closed count', (s) => { s.properties.counts.properties.closed_residual_classes.const = 5; }],
  ['schema open count', (s) => { s.properties.counts.properties.open_residual_classes.const = 37; }],
