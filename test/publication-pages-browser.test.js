...
try {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    ...
    const rootResult = await page.goto(`http://127.0.0.1:${port}/`, { waitUntil: 'networkidle' });
    assert.equal(rootResult.status(), 200);
    assert.equal(await page.locator('[data-network-mode="research"]').count(), 0);
    assert.equal(await page.locator('#network-atlas').count(), 1);
    const publicIndex = fs.readFileSync(path.join(site, 'index.html'), 'utf8');
    const publicApp = fs.readFileSync(path.join(site, 'app.js'), 'utf8');
    assert.match(publicIndex, /data-network-mode="hops"/);
    assert.match(publicApp, /valid hops/);
    const forbiddenRequests = requests.filter(value => value.endsWith('/graph.json') || value.includes('/legacy/'));
