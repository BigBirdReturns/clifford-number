...
} from '../tools/validate-status-sovereignty-residual-denominator-wave-03-constitution.mjs';
...
const cases = [
  ['wave identity', (v) => { v.wave_id = 'SSC-RD-W02'; }, /wave identity/],
  ['issue custody', (v) => { v.issue = 999; }, /issue or date/],
  ['promotion merge', (v) => { v.parent_custody.wave_02_promotion_merge = '0'.repeat(40); }, /promotion merge/],
  ['launch head', (v) => { v.parent_custody.launch_main_head = '0'.repeat(40); }, /launch main head/],
  ['launch paths', (v) => { v.parent_custody.launch_main_advancement_paths.pop(); }, /advancement paths/],
  ['launch disjointness', (v) => { v.parent_custody.launch_main_advancement_disjoint = false; }, /disjointness/],
  ['parent closure count', (v) => { v.parent_custody.closed_residual_classes = 5; }, /cumulative accounting/],
  ['parent open count', (v) => { v.parent_custody.open_residual_classes = 37; }, /cumulative accounting/],
  ['starting closure', (v) => { v.closure_contract.starting_closed_classes = 0; }, /starting class accounting/],
  ['attempt ceiling', (v) => { v.closure_contract.maximum_closed_classes_after_wave = 42; }, /attempt ceiling/],
  ['minimum open floor', (v) => { v.closure_contract.minimum_open_classes_after_full_success = 0; }, /minimum open-class floor/],
  ...
  ['canonical ordinal', (v) => { v.lane_attempts[2].canonical_ordinal = 4; }, /canonical ordinal/],
  ['child issue', (v) => { v.lane_attempts[3].issue = 999; }, /issue changed/],
  ['execution branch', (v) => { v.lane_attempts[4].branch = 'agent/other'; }, /branch changed/],
  ['source binding', (v) => { v.lane_attempts[3].source_path = v.lane_attempts[2].source_path; }, /source binding/],
  ['class label', (v) => { v.lane_attempts[4].exact_label = 'member participation'; }, /exact class label/],
  ...
  ['historical closure rewrite', (v) => { v.counts.classes_closed_before_wave = 0; }, /historical closure count/],
  ['Wave 03 class accounting', (v) => { v.counts.classes_closed_this_wave = 1; }, /class accounting promoted/],
  ...
  ['result closure inflation', (v) => { v.current_result.classes_closed = 12; }, /result class accounting/],
  ...
  ['boundary weakening', (v) => { v.boundaries.source_unavailability_is_event_absence_or_noncompliance = true; }, /source_unavailability_is_event_absence_or_noncompliance/]
];

for (const [name, mutate, pattern] of cases) expectFailure(name, mutate, pattern);
console.log(`status-sovereignty-residual-denominator-wave-03-constitution.test: positive plus ${cases.length} adversarial mutations passed`);
