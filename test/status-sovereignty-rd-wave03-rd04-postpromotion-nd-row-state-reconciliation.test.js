...
test('standing qualification workflow parses as YAML',()=>{
...
test('closed product contract rejects 49 adversarial mutations',()=>{
  const mutations=[
    ...
    ['input promoted matrix Git blob custody drift',mutateJson(rel.input,o=>{o.projection.promoted_matrix_git_blob='0'.repeat(40);})],
  ...
  assert.equal(mutations.length,49);
