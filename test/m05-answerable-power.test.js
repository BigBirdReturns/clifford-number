...
assert.equal(report.counts.stories,15);
assert.equal(report.counts.lanes,18);
...
assert.equal(report.counts.constitutional_mechanism,5);
...
assert.equal(report.boundaries.steel_mirror_differential_proves_bad_faith,false);
for(const id of ['M05-S01','M05-S02','M05-S03','M05-S04','M05-S05','M05-S06','M05-S10','M05-S12','M05-S13','M05-S14','M05-S15'])assert.ok(ids.has(id),id);
