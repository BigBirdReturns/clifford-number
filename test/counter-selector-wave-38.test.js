...
} from '../tools/validate-counter-selector-wave-38.mjs';

const source = JSON.parse(fs.readFileSync('data/project/counter-selector-wave-38-portable-operational-checkpoint.json', 'utf8'));
...
assert.equal(mutations.length, 162);
for (const [index, mutate] of mutations.entries()) {
  const candidate = clone(source);
  mutate(candidate);
  assert.throws(() => validateSource(candidate), undefined, `mutation ${index + 1} should fail`);
}
console.log(`counter-selector-wave-38.test: ${mutations.length} adversarial mutations PASS`);
