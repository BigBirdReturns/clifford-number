...
test('evidence warnings: surfaces receipts with archival distress and review-required claims', () => {
  const fixture = buildFixtureState();
  fixture.receipts = new Map(fixture.receipts);
  const syntheticLost = {
    receipt_id: 'synthetic-unrecoverable-receipt',
    label: 'Synthetic unrecoverable receipt',
    evidence_class: 'reported',
    archive: {
      method: 'unrecoverable_local_paste',
      note: 'Synthetic archival failure used only to exercise the warning contract.'
    }
  };
  fixture.receipts.set(syntheticLost.receipt_id, syntheticLost);
  const model = evidenceOverviewModel(fixture, { activeTab: 'warnings' });
  const lostRow = model.rows.find(r => r.objectId === syntheticLost.receipt_id);
  assert.ok(lostRow, 'an unrecoverable receipt must produce a warning row');
  assert.equal(lostRow.severity, 'lost');
  assert.equal(lostRow.inferenceBoundary, syntheticLost.archive.note);
