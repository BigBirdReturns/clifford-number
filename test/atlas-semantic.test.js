...
const {
  atlasScale, semanticLevel, hopNetworkModel, researchNetworkModel, legacyNetworkModel,
...
for (const fn of [
  atlasScale, semanticLevel, hopNetworkModel, researchNetworkModel, legacyNetworkModel, buildAtlasAggregates,
...
test('current research topology is derived from the current bounded-surface ledger', () => {
  const model = researchNetworkModel();
  assert.equal(model.mode, 'research');
  assert.equal(model.dataBoundary, surfaceGraph.generated);
  const expectedEdges = surfaceGraph.surfaces.reduce((sum, surface) => sum + (surface.participants ?? []).filter(participant => participant.actor_id || participant.organization_id).length, 0);
  assert.equal(model.edges.length, expectedEdges, 'one topology edge must represent each published participation row');
  for (const edge of model.edges) {
    const from = model.nodeById.get(edge.from);
    const to = model.nodeById.get(edge.to);
    assert.ok(from && to, `${edge.id} must resolve both endpoints`);
    assert.equal(to.type, 'surface', `${edge.id} must terminate on a bounded surface`);
    assert.equal(edge.surface_id, edge.to, `${edge.id} must name its surface endpoint`);
    assert.ok(['person', 'organization'].includes(from.type), `${edge.id} must begin at an actor or organization`);
    assert.ok((edge.receipt_ids ?? []).length, `${edge.id} must carry receipts`);
    assert.notEqual(from.type, 'surface', `${edge.id} cannot manufacture surface-to-surface adjacency`);
  }
});

test('legacy broad edges remain a separately declared projection', () => {
  const current = researchNetworkModel();
  const legacy = legacyNetworkModel();
  assert.equal(legacy.mode, 'legacy');
  assert.equal(legacy.nodes.length, legacyGraph.nodes.length);
  assert.equal(legacy.edges.length, legacyGraph.edges.length);
  assert.notEqual(current.dataBoundary, legacyGraph.generated, 'the current model must not inherit the legacy declaration date');
  assert.notEqual(current.mode, legacy.mode);
});
...
test('every rendered aggregate metric carries a named metric word, never a bare digit, per the honesty invariant', () => {
  ...
  for (const agg of aggregates) {
    ...
    assert.match(metricMatch[1], /edge(?:s)?|surfaces|actors/, 'aggregate metric text must name its unit, never render a bare number');
