...
} from '../tools/lib/preference-custody-manifest-v31.mjs';

execFileSync(process.execPath, ['tools/compile-preference-custody-manifest-v31.mjs'], { stdio: 'pipe' });
const manifest = JSON.parse(
  readFileSync('data/research/preference-custody/control-manifest-v31.json', 'utf8')
);
const compiled = JSON.parse(
  readFileSync('build/research/preference-custody-laboratory-floor-v31.json', 'utf8')
);
const markdown = readFileSync(
  'build/research/preference-custody-laboratory-floor-v31.md',
  'utf8'
);
...
assert.equal(compiled.schema_version, 'preference-custody-control-manifest-v31-build@1');
assert.equal(compiled.manifest_id, 'preference-custody-laboratory-floor-v31');
assert.equal(compiled.status, 'laboratory_floor_v31_qualified');
...
assert.deepEqual(
  ...
  Array.from({ length: 33 }, (_, index) => `PC-${String(index + 1).padStart(2, '0')}`).sort()
);

assert.equal(compiled.composition.base_manifest_id, 'preference-custody-laboratory-floor-v30');
assert.equal(compiled.composition.base_schema_version, 'preference-custody-control-manifest-v30-build@1');
assert.equal(compiled.composition.base_control_count, 32);
assert.equal(compiled.composition.extension_control_id, 'PC-33');
...
assert.ok(
  compiled.promotion_boundary.real_case_requires.includes(
    'identity_boundary_v31_observed_record_source_system_namespace_and_denominator'
  )
);
assert.ok(
  compiled.promotion_boundary.real_case_requires.includes(
    'identity_boundary_v31_record_identity_boundary_frame_membership_lineage_consequence_correction_and_interpretation_chain'
  )
);
assert.equal(
...
const pc33 = compiled.controls.find(control => control.control_id === 'PC-33');
assert.ok(pc33);
assert.equal(pc33.fixture_id, 'same-identity-verified-status-different-provenance-v1');
assert.equal(
  pc33.failure_class,
  'identity_resolution_entity_boundary_network_frame_population_denominator_and_membership_succession_equifinality'
);
assert.equal(pc33.graph_effect, 'none');
assert.equal(pc33.counts_toward_thesis_evidence, false);
assert.equal(pc33.conclusion_generated, false);
assert.equal(pc33.real_world_effect_claimed, false);
assert.equal(pc33.preference_change_present, false);
assert.equal(pc33.manipulative_intent_inferable, false);
...
for (const [key, value] of Object.entries(expectedMetrics)) {
  assert.equal(pc33.proof_summary[key], value, `PC-33 ${key}`);
}
for (const key of [
...
]) assert.equal(pc33.proof_summary[key], false);
assert.equal(
  pc33.proof_summary.complete_identity_boundary_assurance_supported_in_at_least_one_world,
  true
);

assert.equal(compiled.custody_chain.length, 5);
assert.equal(compiled.custody_chain[0].event_type, 'qualified_v30_floor_snapshot');
assert.equal(
  compiled.custody_chain[1].event_type,
  'pc33_record_identity_entity_boundary_frame_membership_and_denominator_control_admitted'
);
...
assert.match(markdown, /Preference custody laboratory floor v31/);
assert.match(markdown, /Controls:\*\* 33/);
assert.match(markdown, /PC-33: identity resolution, entity-boundary, and network frame/);
...
assert.ok(validatePreferenceCustodyManifestV31(graphLeak).some(error => /graph_effect/.test(error)));

const issueLeak = structuredClone(manifest);
issueLeak.control_issue = 769;
assert.ok(validatePreferenceCustodyManifestV31(issueLeak).some(error => /control issue/.test(error)));

const baseLeak = structuredClone(manifest);
baseLeak.base_floor.expected_control_count = 27;
assert.ok(validatePreferenceCustodyManifestV31(baseLeak).some(error => /base floor contract/.test(error)));

const frontierLeak = structuredClone(manifest);
frontierLeak.frontier_transition.resolved_base_frontier = 'wrong_frontier';
assert.ok(validatePreferenceCustodyManifestV31(frontierLeak).some(error => /resolved frontier/.test(error)));

const successorLeak = structuredClone(manifest);
successorLeak.frontier_transition.successor_frontiers.pop();
assert.ok(validatePreferenceCustodyManifestV31(successorLeak).some(error => /successor frontiers/.test(error)));

const requirementLeak = structuredClone(manifest);
requirementLeak.real_case_requirements_added.pop();
assert.ok(validatePreferenceCustodyManifestV31(requirementLeak).some(error => /real-case requirements/.test(error)));
...
assert.ok(validatePreferenceCustodyManifestV31(requirementFormatLeak).some(error => /lowercase underscore/.test(error)));

const buildCountLeak = structuredClone(compiled);
buildCountLeak.control_count = 32;
assert.ok(validatePreferenceCustodyManifestV31Build(buildCountLeak).some(error => /thirty-three controls/.test(error)));

const proofLeak = structuredClone(compiled);
proofLeak.controls.find(control => control.control_id === 'PC-33')
  .proof_summary.total_false_merged_entities = 19;
assert.ok(validatePreferenceCustodyManifestV31Build(proofLeak).some(error => /PC-33 total_false_merged_entities/.test(error)));

const integrityLeak = structuredClone(compiled);
integrityLeak.control_integrity.base_integrity_preserved = false;
assert.ok(validatePreferenceCustodyManifestV31Build(integrityLeak).some(error => /base_integrity_preserved/.test(error)));

const resolvedFrontierLeak = structuredClone(compiled);
resolvedFrontierLeak.open_frontiers.push(
  'identity_resolution_entity_boundary_and_network_frame_assurance'
);
assert.ok(validatePreferenceCustodyManifestV31Build(resolvedFrontierLeak).some(error => /resolved identity-boundary frontier/.test(error)));
...
assert.ok(validatePreferenceCustodyManifestV31Build(edgeFrontierLeak).some(error => /preserved base frontier missing/.test(error)));
...
assert.ok(validatePreferenceCustodyManifestV31Build(authorityLeak).some(error => /must remain false/.test(error)));

const chainLeak = structuredClone(compiled);
chainLeak.custody_chain[2].payload.open_frontiers.push('tampered_frontier');
assert.ok(validatePreferenceCustodyManifestV31Build(chainLeak).some(error => /hash mismatch/.test(error)));

const headLeak = structuredClone(compiled);
headLeak.custody_chain_head_sha256 = '0'.repeat(64);
assert.ok(validatePreferenceCustodyManifestV31Build(headLeak).some(error => /custody head mismatch/.test(error)));

console.log('Preference custody laboratory floor v31 integration tests: PASS');
