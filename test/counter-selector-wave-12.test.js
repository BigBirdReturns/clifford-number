...
import { validateContractObject } from '../tools/validate-counter-selector-wave-12.mjs';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');
const original=JSON.parse(fs.readFileSync(path.join(root,'data/project/counter-selector-wave-12-batch-05.json'),'utf8'));
const sources=JSON.parse(fs.readFileSync(path.join(root,'data/project/counter-selector-wave-12-source-registry.json'),'utf8'));
...
console.log(`counter-selector-wave-12.test: PASS (${passed} adversarial mutations)`);
