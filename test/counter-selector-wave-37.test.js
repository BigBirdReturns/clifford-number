...
import { validateSource, EXPECTED_ADJUDICATIONS, EXPECTED_BOUNDARIES, EXPECTED_COUNTS, EXPECTED_JOIN_MATRIX } from '../tools/validate-counter-selector-wave-37.mjs';

const source = JSON.parse(fs.readFileSync('data/project/counter-selector-wave-37-live-operational-handoff.json', 'utf8'));
...
assert.equal(mutations.length, 112);
for (const [index, mutate] of mutations.entries()) {
  const candidate = clone(source);
  mutate(candidate);
  assert.throws(() => validateSource(candidate), undefined, `mutation ${index + 1} should fail`);
}
console.log(`counter-selector-wave-37.test: ${mutations.length} adversarial mutations PASS`);
