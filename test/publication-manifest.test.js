...
const plan = {
  ...
  entries: [
    ...
    { path: 'reports/index.html', kind: 'file', status: 'public' },
    { path: 'build/cases/C1.json', kind: 'file', status: 'public' },
  ],
  catalog_guards: [
  ...
  normalizations: [{ normalization_id: 'FIXTURE-META-01', paths: ['build/cases/C1.json'], field: 'generated', value: '2026-07-29T00:00:00.000Z', publication_status: 'public_normalized_projection', authority_ceiling: 'publication_projection_only_not_evidence_authority' }],
  held_surfaces: [
...
write('index.html', '<link rel="stylesheet" href="assets/site.css"><script src="app.js"></script><script>const card = `<a href="${dynamic}">dynamic</a>`;</script>');
write('app.js', "fetch('build/public-catalog.json'); import('./assets/runtime.js');");
write('assets/runtime.js', "export const ok = true;");
...
write('build/cases/C1.json', JSON.stringify({ generated: 'volatile-now', ok: true }));
write('reports/index.html', '<a href="../#case/C1">root route</a>');
...
for (const relative of ['index.html', 'app.js', 'assets/site.css', 'assets/paper.png', 'build/public-catalog.json', 'build/cases/C1.json', 'reports/index.html', '.nojekyll']) {
...
assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'dist', 'build/cases/C1.json'), 'utf8')).generated, '2026-07-29T00:00:00.000Z');
...
// A retired generic-graph request is invalid even when it is hidden in a
// classified nested public runtime rather than the root app.
write('assets/runtime.js', "fetch('../graph.json');");
assert.throws(
  () => buildPublicationArtifact({ root, destination: path.join(root, 'retired-runtime-dist') }),
  /not classified|forbidden public boundary/
);
write('assets/runtime.js', "export const ok = true;");
