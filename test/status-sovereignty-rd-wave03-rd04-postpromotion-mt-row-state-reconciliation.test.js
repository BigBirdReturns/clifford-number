...
test('external qualification ref pins the reviewed product object and modes',()=>{const w=bytes(rel.workflow).toString();assert.match(w,/QUALIFIED_PRODUCT_REF: refs\/heads\/lock\/ssc-rd04-mt-row-state-reconciliation-v4-qualified/);assert.match(w,/refs\/remotes\/origin\/qualified-product/);assert.match(w,/git rev-parse refs\/remotes\/origin\/qualified-product/);assert.match(w,/git ls-tree \"\$PRODUCT_COMMIT\"/);assert.match(w,/100644 blob/);});
