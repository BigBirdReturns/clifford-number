...
const mutations = [
  ...
  (v) => { v.counts.open_recommendation_disposition_chains = 5; },
  ...
  (v) => { v.source_product.same_object_recovery_index_sha256 = '0'.repeat(64); },
  (v) => { v.source_product.same_object_recoveries_applied = 0; },
  (v) => { v.classification_contract.same_object_recovery_requires_exact_document_binding = false; },
  (v) => { v.classification_contract.source_access_recovery_does_not_expand_denominator = false; },
  (v) => { v.counts.source_access_interstitial_rows = 1; },
  (v) => { v.counts.same_object_recovered_notice_rows = 0; },
  (v) => { v.counts.successor_action_rows = 8; },
  (v) => { v.objects[50].record_class = 'federal_register_access_interstitial'; },
  (v) => { v.objects[50].evidence.source_representation = 'frozen_exact_capture'; },
  (v) => { v.objects[50].fields.exact_source_locator_and_byte_custody.value.source_access_recovered = false; },
  (v) => { v.objects[50].fields.exact_source_locator_and_byte_custody.value.original_representation.semantic_target_delivered = true; },
  (v) => { v.objects[50].fields.exact_source_locator_and_byte_custody.value.same_object_binding.api_document_number_match = false; },
  (v) => { v.objects[50].fields.recommendation_state.value.status = 'completed_recommendation'; },
  (v) => { v.objects[50].fields.recommendation_state.terminal = false; },
  (v) => { v.objects[50].successor_actions.push({ action_type: 'retrieve_same_object_via_federal_register_api', cluster_id: 'FR-2024-08630', blocking: false }); },
  (v) => { v.current_result.same_object_source_recoveries_applied = 0; },
  (v) => { v.current_result.source_access_successor_actions_open = 1; },
  (v) => { v.authority.same_object_source_recoveries_applied = 0; },
