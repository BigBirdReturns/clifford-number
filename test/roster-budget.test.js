...
const denseSurface = [...surfaceGraph.surfaces]
  .sort((a, b) => actorParticipants(b).length - actorParticipants(a).length)
  .find(surface => actorParticipants(surface).length > 100);
...
// ---------------------------------------------------------------------------
// Real build/surface-graph.json fixture (current densest public roster)

test('the current corpus exposes a dense public roster surface', () => {
  assert.ok(denseSurface, 'expected build/surface-graph.json to contain a roster with more than 100 actors');
