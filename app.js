...
function entityHeading(label, receiptIds = []) {
  ...
  return `<div class="entity-heading"><h1>${esc(label)}</h1><div class="entity-actions">${citationActions()}</div></div>`;
}
...
function atlasInspectorOrigin(el) {
  ...
  return {
    mode: model.mode === 'hops' ? 'Verified surface hops' : model.mode === 'legacy' ? 'Legacy edge context' : 'Current surface topology',
...
const TOPOLOGY_CLUSTER_CENTERS = Object.freeze({
  policy: [260, 215], governance: [690, 180], defense: [1110, 230],
  admission: [270, 655], capital: [720, 675], procurement: [1110, 620],
  context: [700, 445]
});
const TOPOLOGY_CLUSTER_ORDER = Object.freeze(['policy', 'governance', 'admission', 'capital', 'procurement', 'defense', 'context']);
const TOPOLOGY_SURFACE_CLUSTER = Object.freeze({
  policy_document_surface: 'policy', government_advisory_surface: 'policy',
  model_governance_surface: 'governance', category_formation_surface: 'governance',
  directory_roster_surface: 'admission', founder_officer_surface: 'admission',
  board_advisory_surface: 'admission', campaign_surface: 'admission',
  employment_investment_surface: 'capital', public_private_fund_surface: 'capital',
  procurement_contract_surface: 'procurement', customer_vendor_surface: 'procurement',
  defence_industrial_surface: 'defense', laundering_chain_connector: 'context'
});
...
function topologySurfaceCluster(surface) {
  const primary = TOPOLOGY_SURFACE_CLUSTER[surface?.surface_type];
  if (primary) return primary;
  const secondary = surface?.secondary_surface_types ?? [];
  if (secondary.includes('defence_industrial_surface')) return 'defense';
  if (secondary.includes('surface_factory_capital_layer')) return 'capital';
  if (secondary.includes('policy_to_procurement_surface')) return 'procurement';
  if (secondary.includes('model_governance_surface')) return 'governance';
  return 'context';
}

function topologyDominantCluster(clusterCounts) {
  return [...clusterCounts.entries()].sort((a, b) =>
    b[1] - a[1]
    || TOPOLOGY_CLUSTER_ORDER.indexOf(a[0]) - TOPOLOGY_CLUSTER_ORDER.indexOf(b[0])
    || a[0].localeCompare(b[0])
  )[0]?.[0] ?? 'context';
}

function topologyPosition(id, cluster, kind) {
  const fixed = {
    dialog: [270, 625], 'matt-clifford': [260, 170],
    'ai-opportunities-action-plan': [340, 245], palantir: [1080, 575]
  };
  if (fixed[id]) return { x: fixed[id][0], y: fixed[id][1] };
  const center = TOPOLOGY_CLUSTER_CENTERS[cluster] ?? TOPOLOGY_CLUSTER_CENTERS.context;
  const h = topologyHash(`${kind}:${id}`);
  const angle = (h / 0xffffffff) * Math.PI * 2;
  const fraction = ((h >>> 10) & 0xffff) / 0xffff;
  const bounds = kind === 'surface' ? [45, 155] : kind === 'organization' ? [105, 245] : [175, 330];
  const radius = bounds[0] + fraction * (bounds[1] - bounds[0]);
  return {
    x: Math.max(28, Math.min(1372, center[0] + Math.cos(angle) * radius)),
    y: Math.max(28, Math.min(872, center[1] + Math.sin(angle) * radius))
  };
}

// Current authoritative topology. Unlike the retained legacy edge graph, this
// model is rebuilt in the browser from the release's surface ledger. Its
// geometry is bipartite: actor/organization -> bounded surface. No pairwise
// actor adjacency is invented here; verified actor-to-actor hops remain the
// separate compiler-owned `hops` mode.
function researchNetworkModel() {
  const graph = state.surfaceGraph ?? { actors: [], organizations: [], surfaces: [] };
  const actorById = new Map((graph.actors ?? []).map(actor => [actor.id, actor]));
  const orgById = new Map((graph.organizations ?? []).map(org => [org.id, org]));
  const nodeSeed = new Map();
  const clusterCountsByNode = new Map();
  const edges = [];

  const ensureNode = (id, kind, record, cluster) => {
    if (!nodeSeed.has(id)) nodeSeed.set(id, { id, kind, record, cluster });
    if (!clusterCountsByNode.has(id)) clusterCountsByNode.set(id, new Map());
    const counts = clusterCountsByNode.get(id);
    counts.set(cluster, (counts.get(cluster) ?? 0) + 1);
  };

  for (const surface of [...(graph.surfaces ?? [])].sort((a, b) => a.surface_id.localeCompare(b.surface_id))) {
    const cluster = topologySurfaceCluster(surface);
    ensureNode(surface.surface_id, 'surface', surface, cluster);
    for (const [index, participant] of (surface.participants ?? []).entries()) {
      const isActor = participant.participant_type === 'actor';
      const participantId = isActor ? participant.actor_id : participant.organization_id;
      if (!participantId) continue;
      const participantRecord = isActor ? actorById.get(participantId) : orgById.get(participantId);
      ensureNode(participantId, isActor ? 'person' : 'organization', participantRecord ?? { id: participantId, label: participantId }, cluster);
      const receipts = [...new Set([...(participant.receipt_ids ?? []), ...(surface.receipt_ids ?? [])])];
      edges.push({
        id: `participation-${surface.surface_id}-${participantId}-${index}`,
        from: participantId, to: surface.surface_id,
        type: 'bounded-surface-participation',
        evidence_class: participant.evidence_class ?? 'open',
        source_ids: receipts, receipt_ids: receipts,
        surface_id: surface.surface_id, surface_label: surface.surface_label,
        participant_type: participant.participant_type,
        role: participant.role ?? participant.participation_type ?? 'documented participant',
        participation_type: participant.participation_type ?? null,
        claim: `${participantRecord?.label ?? participantId}: ${participant.role ?? participant.participation_type ?? 'documented participant'} on ${surface.surface_label}.`
      });
    }
  }
  ...
  const nodes = [...nodeSeed.values()].sort((a, b) => a.id.localeCompare(b.id)).map(seed => {
    const cluster = seed.kind === 'surface' ? seed.cluster : topologyDominantCluster(clusterCountsByNode.get(seed.id));
    const record = seed.record ?? {};
    const label = record.label ?? record.surface_label ?? seed.id;
    const description = record.notes ?? record.description ?? record.plain?.who ?? null;
    return {
      id: seed.id, label, description,
      type: seed.kind, cluster, aggregateGroup: cluster,
      degree: degree.get(seed.id) ?? 0,
      recordKind: seed.kind === 'person' ? 'actor' : seed.kind,
      recordId: seed.kind === 'surface' ? record.surface_id : seed.id,
      ...topologyPosition(seed.id, cluster, seed.kind)
    };
  });
  return {
    mode: 'research', dataBoundary: graph.generated ?? null,
    nodes, edges, nodeById: new Map(nodes.map(node => [node.id, node])),
    defaultNode: nodes.some(node => node.id === 'dialog') ? 'dialog' : nodes[0]?.id,
    fullView: { ...NETWORK_FULL_VIEW }
  };
}

function legacyNetworkModel() {
  ...
  return {
    mode: 'legacy', nodes, edges: graph.edges ?? [], nodeById: new Map(nodes.map(node => [node.id, node])),
...
function renderEdgeLineGlyph(edge, model, subordinationClass = null) {
  ...
  if (!from || !to) return '';
  const band = evidenceBand(edge.evidence_class);
  const topology = model.mode === 'legacy' && legacyIsTopology(edge) ? ' network-edge--topology' : '';
  const subordination = subordinationClass ? ` ${subordinationClass}` : '';
...
function renderNetworkAtlas(mode = state.networkMode, selectedId = null) {
  ...
  state.networkModel = mode === 'hops' ? hopNetworkModel() : mode === 'legacy' ? legacyNetworkModel() : researchNetworkModel();
  const model = state.networkModel;
  state.networkLevel = null;
  const uniqueSurfaces = new Set((state.hopGraph.edges ?? []).flatMap(edge => (edge.surfaces ?? []).map(item => item.surface_id)));
  const stats = model.mode === 'hops'
    ...
    : model.mode === 'legacy'
      ? `<strong>${model.nodes.length}</strong> legacy nodes <span>·</span> <strong>${model.edges.length}</strong> sourced edges <span>·</span> declared ${esc(state.legacyGraph.generated || 'date unknown')}`
      : `<strong>${model.nodes.length}</strong> current topology objects <span>·</span> <strong>${model.edges.length}</strong> receipted participations <span>·</span> built ${esc(model.dataBoundary || 'date unknown')}`;
  $('#atlas-stats').innerHTML = stats;
...
function selectNetworkNode(id) {
  ...
  const cards = related.map(edge => {
    ...
    if (model.mode === 'research') {
      const receipts = edge.receipt_ids ?? edge.source_ids ?? [];
      return `<article class="network-edge-card"><div><span class="badge">${esc(humanLabel(edge.evidence_class || 'open'))}</span><strong>${esc(other?.label ?? otherId)}</strong></div><p><b>${esc(edge.surface_label || 'Bounded surface')}</b><br><span>${esc(edge.role || edge.claim || 'documented participation')}</span></p><div class="network-receipt-buttons">${receipts.slice(0, 3).map(receiptId => `<button type="button" data-open-receipt="${esc(receiptId)}">Receipt · ${esc(shortLabel(receiptId, 28))}</button>`).join('')}</div><button type="button" class="result edge-evidence-button" data-kind="surface" data-id="${esc(edge.surface_id)}">Open bounded surface →</button></article>`;
    }
  ...
  const modeLabel = model.mode === 'hops' ? 'Verified surface-hop node' : model.mode === 'legacy' ? 'Legacy edge context' : 'Current bounded topology';
  const recordKind = node.recordKind ?? (model.mode === 'hops' ? 'actor' : state.actors?.has(node.id) ? 'actor' : state.orgs?.has(node.id) ? 'organization' : state.surfaces?.has(node.id) ? 'surface' : null);
  const recordId = node.recordId ?? node.id;
  const glyph = recordKind === 'organization' ? 'O' : recordKind === 'surface' ? 'S' : 'A';
  const profileAction = recordKind ? `<button class="result network-profile-link" data-kind="${esc(recordKind)}" data-id="${esc(recordId)}"><span class="kind-glyph">${glyph}</span><span class="result-label">Open the full record<small>roles, windows, and receipts</small></span></button>` : '';
  inspector.innerHTML = `<p class="section-kicker">${esc(modeLabel)}</p><h3>${esc(node.label)}</h3><div class="network-node-metric"><strong>${node.degree}</strong><span>documented edge${node.degree === 1 ? '' : 's'}</span></div>${node.description ? `<p>${esc(node.description)}</p>` : ''}${profileAction}<h4>Strongest visible edges</h4><div class="network-edge-list">${cards || '<p>No edge is visible in this view.</p>'}</div>`;
...
function openNetworkEdge(id, el = null) {
  ...
  if (state.networkMode === 'research') {
    const receiptId = (edge.receipt_ids ?? edge.source_ids ?? [])[0];
    if (receiptId) openReceiptDialog(receiptId, atlasInspectorOrigin(el));
    else if (edge.surface_id) selectNetworkNode(edge.surface_id);
    return;
  }
  selectNetworkNode(edge.from);
}

function focusNetworkNode(id) {
  const current = state.networkModel;
  if (!current?.nodeById.has(id)) {
    const currentModel = researchNetworkModel();
    renderNetworkAtlas(currentModel.nodeById.has(id) ? 'research' : 'legacy', id);
  } else {
...
function renderNotFound(kind, id) {
  ...
  $('#summary').innerHTML = '';
  $('#detail').innerHTML = `<div class="panel why-no-hop"><span class="panel-label">Stale or unknown link</span><h1>Not found in this release.</h1><p>The ${esc(kind || 'item')} “${esc(id || '')}” is not present in the current public corpus. It may have been renamed, withheld, or never promoted from intake.</p><p><button class="copy-link" type="button" onclick="location.hash=''">Return to the explorer</button></p></div>`;
}
...
async function renderTrack(id) {
  ...
  $('#detail').innerHTML = `
    <article class="panel track-hero">
      <div class="entity-heading"><h1>${esc(track.label)}</h1><div class="entity-actions"><button class="copy-link" type="button" onclick="copyLink(this)">Copy link</button></div></div>
...
init().catch(err => {
  ...
  $('#detail').innerHTML = `<div class="panel why-no-hop"><h1>Load error</h1><p>${esc(err.message)}</p><p>Run <code>npm run compile</code>, then reload this page.</p></div>`;
});
